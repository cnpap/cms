...
function morenews_filter_font_variants($font)
{
	...
	[$font_name, $variants] = explode(':', $font);

	$allowed_variants = ['400', '700'];
...
function morenews_get_fonts_url()
{
	...
	$all_fonts = [$site_title_font, $primary_font, $secondary_font];

	$theme_fonts = array_filter(
...
function morenews_preload_google_fonts()
{
	...
	$fonts_in_use = [$site_title_font, $primary_font, $secondary_font];
	...
	foreach ($fonts_in_use as $font) {
		...
		if (
			strpos($font_clean, 'open+sans') !== false ||
			strpos($font_clean, 'open sans') !== false
		) {
			$load_open_sans = true;
		}
...
function morenews_enqueue_fonts()
{
	...
	$fonts_url = morenews_get_fonts_url();
	if ($fonts_url) {
		wp_enqueue_style('morenews-google-fonts', $fonts_url, [], null);
	}
}
...
function morenews_style_files()
{
	...
	wp_enqueue_style('aft-icons', get_template_directory_uri() . '/assets/icons/style.css', []);
	...
	wp_enqueue_style(
		'slick',
		get_template_directory_uri() . '/assets/slick/css/slick' . $min . '.css',
	);

	wp_enqueue_style(
		'sidr',
		get_template_directory_uri() . '/assets/sidr/css/jquery.sidr.dark.css',
	);

	wp_enqueue_style(
		'magnific-popup',
		get_template_directory_uri() . '/assets/magnific-popup/magnific-popup.css',
	);
...
function morenews_scripts()
{
	...
	wp_enqueue_script(
		'morenews-toggle-script',
		get_template_directory_uri() . '/assets/toggle-script.js',
		['jquery-cookie'],
		$morenews_version,
		true,
	);
	...
	wp_enqueue_script(
		'morenews-background-script',
		get_template_directory_uri() . '/assets/background-script.js',
		['jquery'],
		$morenews_version,
	);
	wp_enqueue_script(
	...
	wp_enqueue_script(
		...
		get_template_directory_uri() .
			'/assets/magnific-popup/jquery.magnific-popup' .
			$min .
			'.js',
		['jquery'],
		$morenews_version,
	...
	wp_enqueue_script(
		'matchheight',
		get_template_directory_uri() .
			'/assets/jquery-match-height/jquery.matchHeight' .
			$min .
			'.js',
		['jquery'],
		$morenews_version,
...
function morenews_header_menu_desc($item_output, $item, $depth, $args)
{
	...
	if ($show_primary_menu_desc) {
		if (
			...
			'aft-primary-nav' == $args->theme_location &&
			$item->description
		) {
...
function athfb_load_files()
{
	...
	require_once get_template_directory() .
		'/inc/customizer/builder/class-header-footer-builder.php';
	require_once get_template_directory() .
		'/inc/customizer/builder/class-header-footer-builder-control.php';
