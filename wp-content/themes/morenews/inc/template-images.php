...
if (!function_exists('morenews_post_thumbnail')):
	...
	function morenews_post_thumbnail()
	{
		...
		if (is_singular()):
			...
			$morenews_post_image_alignment = get_post_meta(
				$post->ID,
				'morenews-meta-image-options',
				true,
			);
			$morenews_post_class = !empty($morenews_post_image_alignment)
				? $morenews_post_image_alignment
				: $morenews_theme_class;

			if ($morenews_post_class != 'no-image'):
				$single_featured_image_view = morenews_get_option('single_featured_image_view'); ?>
        <div class="post-thumbnail <?php echo esc_attr(
        	$morenews_post_class,
        ); ?> <?php echo esc_attr($single_featured_image_view); ?>">
    ...
    <?php
			...
			$morenews_archive_image =
				$morenews_archive_layout == 'archive-layout-list'
					? 'medium'
					: ($morenews_archive_layout == 'archive-layout-full'
						? 'morenews-medium'
						: 'post-thumbnail');
			$morenews_archive_class =
				$morenews_archive_layout == 'archive-layout-list'
					? morenews_get_option('archive_image_alignment')
					: '';
			?>
      <div class="post-thumbnail <?php echo esc_attr($morenews_archive_class); ?>">
        <a href="<?php the_permalink(); ?>" aria-label="<?php echo esc_attr(
	get_the_title($post->ID, 'morenews'),
); ?>" aria-hidden="true">
...
if (!function_exists('morenews_the_post_thumbnail')):
	 ...
	 */
	function morenews_the_post_thumbnail(
		...
		if ($morenews_fetch_content_image == 'enable') {
			if (has_post_thumbnail($morenews_post_id)) {
				...
				$morenews_post_content = substr(
					get_post_field('post_content', $morenews_post_id),
					0,
					3000,
				);
				$output = preg_match_all(
					'/<img.+src=[\'"]([^\'"]+)[\'"].*>/i',
					$morenews_post_content,
					$matches,
				);

				if (isset($matches[1][0])) {
					$morenews_img_id = morenews_find_post_id_from_path($matches[1][0]);
					$morenews_img_url = wp_get_attachment_image_src(
						$morenews_img_id,
						$morenews_thumbnail_size,
					);
					...
					if (isset($morenews_img_url[0])) {
						if ($return) {
							...
							echo wp_kses_post(
								wp_get_attachment_image($morenews_img_id, $morenews_thumbnail_size),
							);
						}
					} else {
						...
						if (@getimagesize($matches[1][0])) {
							if ($return) {
								ob_start(); ?>
                <img src="<?php echo esc_url($matches[1][0]); ?>" alt="<?php echo esc_attr(
	basename($matches[1][0]),
); ?>" />
              <?php
	...
	if (empty($attributes['alt'])) {
		...
		if (!empty($attachment->post_title)) {
			...
			if ($post_id) {
				...
				$attributes['alt'] = esc_attr(
					morenews_get_image_alt_from_filename($attachment->guid),
				);
			}
		}
	}
