...
class AMP_Walker_Nav_Menu extends Walker_Nav_Menu
{
	function start_lvl(&$output, $depth = 0, $args = null)
	{
		...
		$output .=
			'<ul class="sub-menu" [class]="menuOpen' .
			$menu_item_id_for_amp .
	...
	function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
	{
		...
		$item_output =
			'<a' . $attributes . ' [class]="menuOpen' . $item->ID . ' ? \'active\' : \'\'">';
		...
		if ($has_children) {
			...
			$item_output .=
				'<button on="tap:AMP.setState({ menuOpen' .
				$item->ID .
				': !menuOpen' .
				$item->ID .
				' })" ';
			$item_output .=
				'[class]="menuOpen' .
				$item->ID .
				' ? \'fa fa-angle-down active\' : \'fa fa-angle-down\'" ';
