...
if (!class_exists('Morenews_AMP_Module')) {
	class Morenews_AMP_Module
	{
		...
		{
			add_action('wp', [$this, 'morenews_init']);
		...
		{
			add_filter('sidebars_widgets', [$this, 'morenws_amp_disable_sidebar_1']);
			...
			remove_action(
				...
				'morenews_full_width_upper_footer_section',
			);
		...
		{
			add_action('wp_enqueue_scripts', [$this, 'morenews_style_amp_files'], 40);
			add_action('morenews_action_main_menu_nav', [$this, 'morenews_amp_custom_header'], 40);
			add_action('morenews_load_search_form', [$this, 'morenews_amp_search_form_section']);
		}
		...
		public function morenews_amp_custom_header()
		{
			...
			if (has_header_image()) {
				if ($select_header_image_mode == 'above') {
					...
					$inline_style =
						'style="background-image: url(' . esc_url($morenews_background) . ');"';
				}
			}
			?>
      ...
      <div class="navigation-container">
        <nav class="main-navigation clearfix">
          ...
          <div class="amp-mobile-menu" [class]="menuToggled ? 'amp-mobile-menu active':''" aria-expanded="false">
            <?php
            $morenews_global_show_home_menu = morenews_get_option(
          ...
          wp_nav_menu([
          	...
          	'container' => 'div',
          	'container_class' =>
          		'menu main-menu menu-desktop ' . esc_attr($morenews_global_show_home_menu),
          ]);?>
    ...
    <?php
      ...
      <div class="search-watch">
        <div class="af-search-wrap">
          <div class="search-overlay" [class]="'search-overlay' + (ampSearch ? ' reveal-search' : '')" aria-label="<?php esc_attr_e(
          	'Open search form',
          	'morenews',
          ); ?>">
            <a href="#" title="Search"
