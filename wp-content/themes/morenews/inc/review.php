...
class MoreNews_Theme_Review_Notice
{
	...
	{
		add_action('after_setup_theme', [$this, 'morenews_theme_rating_notice']);
		add_action('switch_theme', [$this, 'morenews_theme_rating_notice_data_remove']);
	}
	...
	public function morenews_theme_rating_notice()
	{
		...
		add_action('admin_notices', [$this, 'morenews_theme_review_notice'], 0);
		add_action('admin_init', [$this, 'morenews_ignore_theme_review_notice'], 0);
		add_action('admin_init', [$this, 'morenews_ignore_theme_review_notice_partially'], 0);
	}
	...
	public function morenews_theme_review_notice()
	{
		...
		$ignored_notice_partially = get_user_meta(
			$user_id,
			'nag_morenews_ignore_theme_review_notice_partially_v3',
			true,
		);

		// Return from notice display if conditions are met.
		if (
			get_option('morenews_theme_installed_time_v3') > strtotime('-5 days') ||
			$ignored_notice_partially > strtotime('-2 days') ||
			$ignored_notice
		) {
			return;
		}
		...
		$review_link =
			'https://wordpress.org/support/theme/' .
			strtolower($theme_slug) .
	...
	public function morenews_ignore_theme_review_notice()
	{
		...
		if (
			isset($_GET['nag_morenews_ignore_theme_review_notice_v3']) &&
	...
	public function morenews_ignore_theme_review_notice_partially()
	{
		...
		if (
			isset($_GET['nag_morenews_ignore_theme_review_notice_partially_v3']) &&
			'0' == $_GET['nag_morenews_ignore_theme_review_notice_partially_v3']
		) {
			update_user_meta(
				$user_id,
				'nag_morenews_ignore_theme_review_notice_partially_v3',
	...
	public function morenews_theme_rating_notice_data_remove()
	{
		...
		foreach ($get_all_users as $user) {
			$ignored_notice = get_user_meta(
				$user->ID,
				'morenews_ignore_theme_review_notice_v3',
				true,
			);
			$ignored_notice_partially = get_user_meta(
				$user->ID,
