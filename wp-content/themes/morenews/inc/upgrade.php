...
class MoreNews_Notice
{
	...
	{
		if (
			isset($_GET['morenews_upgrade_notice_dismiss']) &&
			isset($_GET['_morenews_upgrade_notice_dismiss_nonce'])
		) {
			// WPCS: input var ok.
			if (
				!wp_verify_nonce(
					wp_unslash($_GET['_morenews_upgrade_notice_dismiss_nonce']),
			...
			$dismiss_notice = sanitize_text_field(
				wp_unslash($_GET['morenews_upgrade_notice_dismiss']),
			);
			...
			if ($dismiss_notice === $_GET['morenews_upgrade_notice_dismiss']) {
				add_user_meta(
					get_current_user_id(),
					'morenews_' . $dismiss_notice . '_notice_dismiss',
					'yes',
					true,
				);
			}
	...
	{
		if (
			isset($_GET['morenews_upgrade_notice_dismiss_temporary']) &&
			isset($_GET['_morenews_upgrade_notice_dismiss_temporary_nonce'])
		) {
			// WPCS: input var ok.
			if (
				!wp_verify_nonce(
					wp_unslash($_GET['_morenews_upgrade_notice_dismiss_temporary_nonce']),
			...
			$dismiss_notice = sanitize_text_field(
				wp_unslash($_GET['morenews_upgrade_notice_dismiss_temporary']),
			);
			...
			if ($dismiss_notice === $_GET['morenews_upgrade_notice_dismiss_temporary']) {
				add_user_meta(
					get_current_user_id(),
					'morenews_' . $dismiss_notice . '_notice_dismiss_temporary',
					'yes',
					true,
				);
			}
...
class MoreNews_Upgrade_Notice extends MoreNews_Notice
{
	...
	private function set_temporary_dismiss_notice_time()
	{
		if (
			isset($_GET['morenews_upgrade_notice_dismiss_temporary']) &&
			'upgrade' === $_GET['morenews_upgrade_notice_dismiss_temporary']
		) {
			update_user_meta(
				$this->current_user_id,
				'morenews_upgrade_notice_dismiss_temporary_start_time',
				time(),
			);
		}
	}
	...
	public function set_dismiss_notice()
	{
		 ...
		 */
		if (
			get_option('morenews_upgrade_notice_start_time') > strtotime('-2 minutes') ||
	...
	public function notice_markup()
	{
        ...
        <div class="notice notice-success morenews-notice" >
            <a class="morenews-notice-dismiss notice-dismiss" href="<?php echo esc_url(
            	$this->dismiss_url,
            ); ?>"></a>
