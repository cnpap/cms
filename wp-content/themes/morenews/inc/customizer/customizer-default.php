...
if (!function_exists('morenews_get_default_theme_options')):
	...
	function morenews_get_default_theme_options()
	{
		...
		for ($i = 1; $i <= 3; $i++) {
			$defaults['featured_post_list_section_title_' . $i] = sprintf(
				esc_html__('Posts List %d', 'morenews'),
				$i,
			);
		...
		for ($i = 1; $i < 5; $i++) {
			$defaults['featured_category_section_' . $i] = sprintf(
				esc_html__('Post list Section Title %d', 'morenews'),
				$i,
			);
		...
		$defaults['footer_copyright_text'] = esc_html__(
