...
if (!function_exists('morenews_post_format')):
	function morenews_post_format($post_id)
	{
		...
		switch ($post_format) {
			case 'image':
				$post_format =
					"<div class='af-post-format em-post-format'><i class='fas fa-image'></i></div>";
				break;
			case 'video':
				$post_format =
					"<div class='af-post-format em-post-format'><i class='fas fa-play'></i></div>";

				break;
			case 'gallery':
				$post_format =
					"<div class='af-post-format em-post-format'><i class='fas fa-images'></i></div>";
				break;
			default:
				$post_format = '';
		}
...
if (!function_exists('morenews_get_breadcrumb')):
    ...
    <div class="af-breadcrumbs font-family-1 color-pad">
      ...
      <?php if (function_exists('yoast_breadcrumb') && $select_breadcrumbs == 'yoast') {
      	yoast_breadcrumb();
...
add_action(
	'morenews_action_front_page_main_section_scope',
	'morenews_front_page_main_section_scope',
);
	...
	function morenews_count_content_words($post_id)
	{
		...
		if ($morenews_show_read_mins == 'yes') {
			...
			if (absint($morenews_word_per_min) > 0) {
				$word_count_strings = sprintf(
					__('%s min read', 'morenews'),
					number_format_i18n($morenews_word_per_min),
				);
 ...
 * @return Boolean
 */
function morenews_list_popular_taxonomies(
	$taxonomy = 'post_tag',
	$title = 'Popular Tags',
	$number = 5
) {
	$popular_taxonomies = get_terms([
		'taxonomy' => $taxonomy,
...
function morenews_archive_social_share_icons($post_id)
{
	if (class_exists('Jetpack') && Jetpack::is_module_active('sharedaddy')):
		if (function_exists('sharing_display')):
			...
			if (
				in_array('index', $global['show']) &&
				(is_home() ||
					is_front_page() ||
					is_archive() ||
					is_search() ||
					in_array(get_post_type(), $global['show']))
			): ?>
...
function morenews_single_post_social_share_icons()
{
  ...
  <?php if ($social_share_icon_opt == 'side') {
  	echo '</div>';
  }
	endif;
}
...
if (!function_exists('morenews_author_bio_box')):
	function morenews_author_bio_box()
	{
		...
		$allowed_post_types = apply_filters('morenews_author_bio_post_types', ['post']);
          ...
          <div class="author-box-content">
              ...
              <div class="author-info">
                  ...
                  <?php if ($website): ?>
                          ...
                          <a class="author-website" href="<?php echo esc_url(
                          	$website,
