		...
		if ('post' === get_post_type()) {
			...
			if ($morenews_post_categories) {
				...
				foreach ($morenews_post_categories as $post_category) {
					...
					$morenews_color_class = $morenews_term_meta
						? $morenews_term_meta['color_class_term_meta']
						: 'category-color-1';

					$morenews_output .=
						'<li class="meta-category">
                             <a class="morenews-categories ' .
						esc_attr($morenews_color_class) .
						'" href="' .
						esc_url(get_category_link($post_category)) .
						'" aria-label="' .
					...
					if ($morenews_is_single == false) {
						if (++$category_count == $show_category_number) {
							break;
						}
					}
				}
...
if (!function_exists('morenews_post_item_meta')):
	function morenews_post_item_meta($morenews_post_display = 'spotlight-post')
	{
		...
		if ('post' == get_post_type($post->ID)):
			...
			$morenews_author_icon_gravatar_display_setting = morenews_get_option(
            ...
            <?php if ($morenews_display_author): ?>
                <span class="item-metadata posts-author byline">
                    <?php if (
                    	$morenews_author_icon_gravatar_display_setting == 'display-gravatar'
                    ) {
                    	morenews_by_author($gravatar = true);
                    } elseif ($morenews_author_icon_gravatar_display_setting == 'display-icon') { ?>
                        <i class="far fa-user-circle"></i>
                    <?php morenews_by_author($gravatar = false);} else {morenews_by_author(
                    		$gravatar = false,
                    	);} ?>
                </span>
            <?php endif; ?>
            ...
            <?php if ($morenews_display_date): ?>
                <span class="item-metadata posts-date">
                    <i class="far fa-clock" aria-hidden="true"></i>
                    <?php // Get the post's date components (year, month, day) for building the archive link
                    // $year  = get_the_time('Y');
		...
		if ($morenews_single_show_tags_list) {
			if ('post' === get_post_type()) {
				...
				if ($tags_list) {
					...
					printf(
						'<span class="tags-links">' . esc_html('Tags: %1$s') . '</span>',
						$tags_list,
					);
				}
			}
