...
if (!function_exists('morenews_by_author')) {
	function morenews_by_author($gravatar = '')
	{
		...
		if (class_exists('WP_Post_Author')) {
			...
			if (
				$enable_author_metabox_for_post &&
				$enable_author_metabox_for_post['enable_author_metabox'] == true
			) {
				$multiauthor_settings = true;
			}
			if (
				isset($awpa_post_authors) &&
				!empty($awpa_post_authors) &&
				$multiauthor_settings == true
			) {
