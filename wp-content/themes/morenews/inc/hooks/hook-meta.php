...
if (!function_exists('morenews_render_layout_options_metabox')):
	...
	function morenews_render_layout_options_metabox($post, $metabox)
	{
		...
		// Fetch Options list.
		$morenews_content_layout = get_post_meta(
			$morenews_post_id,
			'morenews-meta-content-alignment',
			true,
		);
		$morenews_global_single_content_mode = get_post_meta(
			$morenews_post_id,
			'morenews-meta-content-mode',
			true,
		);
		...
		if (empty($morenews_global_single_content_mode)) {
			$morenews_global_single_content_mode = morenews_get_option(
                ...
                <div class="morenews-row-content">
                    ...
                    <p>
                        <select name="morenews-meta-content-mode" id="morenews-meta-content-mode">
                            ...
                            <option value="single-content-mode-default" <?php selected(
                            	'single-content-mode-default',
                            	$morenews_global_single_content_mode,
                            ); ?>>
                                <?php esc_html_e('Default', 'morenews'); ?>
                            </option>
                            <option value="single-content-mode-boxed" <?php selected(
                    ...
                    <small><?php esc_html_e(
                    	'Please go to Customize>Themes Options for Single Post/Page.',
                    	'morenews',
                    ); ?></small>
                ...
                <div class="morenews-row-content">
                    ...
                    <p>
                        <select name="morenews-meta-content-alignment" id="morenews-meta-content-alignment">
                            ...
                            <option value="align-content-left" <?php selected(
                            	'align-content-left',
                            	$morenews_content_layout,
                            ); ?>>
                                <?php esc_html_e('Content - Primary Sidebar', 'morenews'); ?>
                            </option>
                            <option value="align-content-right" <?php selected(
                            	'align-content-right',
                            	$morenews_content_layout,
                            ); ?>>
                                <?php esc_html_e('Primary Sidebar - Content', 'morenews'); ?>
                            </option>
                            <option value="full-width-content" <?php selected(
                            	'full-width-content',
                            	$morenews_content_layout,
                            ); ?>>
                    ...
                    <small><?php esc_html_e(
                    	'Please go to Customize>Front-page Options for Homepage.',
                    	'morenews',
                    ); ?></small>
...
if (!function_exists('morenews_save_layout_options_meta')):
	...
	function morenews_save_layout_options_meta($morenews_post_id, $post)
	{
		// Verify nonce.
		if (
			!isset($_POST['morenews_meta_box_nonce']) ||
			!wp_verify_nonce($_POST['morenews_meta_box_nonce'], basename(__FILE__))
		) {
			return;
		}
		...
		} elseif (!current_user_can('edit_post', $morenews_post_id)) {
			return;
		}

		$morenews_content_layout = isset($_POST['morenews-meta-content-alignment'])
			? $_POST['morenews-meta-content-alignment']
			: '';
		$morenews_global_single_content_mode = isset($_POST['morenews-meta-content-mode'])
			? $_POST['morenews-meta-content-mode']
			: '';
		update_post_meta(
			$morenews_post_id,
			'morenews-meta-content-alignment',
			sanitize_text_field($morenews_content_layout),
		);
		update_post_meta(
			$morenews_post_id,
			'morenews-meta-content-mode',
			sanitize_text_field($morenews_global_single_content_mode),
		);
	}
endif;
...
if (!function_exists('morenews_taxonomy_add_new_meta_field')):
	...
	function morenews_taxonomy_add_new_meta_field()
	{
        ...
        <div class="form-field">
            <label for="term_meta[color_class_term_meta]"><?php esc_html_e(
            	'Color Class',
            	'morenews',
            ); ?></label>
            <select id="term_meta[color_class_term_meta]" name="term_meta[color_class_term_meta]">
                <?php foreach ($morenews_cat_color as $key => $value): ?>
                    <option value="<?php echo esc_attr($key); ?>"><?php echo esc_html(
	$value,
); ?></option>
                <?php endforeach; ?>
            </select>
            <p class="description"><?php esc_html_e(
            	'Select category color class. You can set appropriate categories color on "Categories" section of the theme customizer.',
            	'morenews',
            ); ?></p>
        </div>
        <?php
...
if (!function_exists('morenews_taxonomy_edit_meta_field')):
	...
	function morenews_taxonomy_edit_meta_field($term)
	{
        ...
        <tr class="form-field">
            <th scope="row" valign="top"><label
                        for="term_meta[color_class_term_meta]"><?php esc_html_e(
                        	'Color Class',
                        	'morenews',
                        ); ?></label></th>
            <td>
                <?php $morenews_cat_color = [
                	'category-color-1' => __('Category Color 1', 'morenews'),
                	'category-color-2' => __('Category Color 2', 'morenews'),
                	'category-color-3' => __('Category Color 3', 'morenews'),
                ]; ?>
	...
	isset($morenews_term_meta['color_class_term_meta'])
		? $morenews_term_meta['color_class_term_meta']
		: '',
	$key,
