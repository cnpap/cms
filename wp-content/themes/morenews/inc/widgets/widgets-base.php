...
class MoreNews_Widget_Base extends WP_Widget
{
	 ...
	 */
	public function morenews_generate_section_title(
		$field,
		$label,
		$note = '',
		$class = '',
	 ...
	 * @param string $note field note to appear below
	 */
	public function morenews_generate_text_input(
		$field,
		$label,
		$value,
		$type = 'text',
        ...
        <p>
            ...
            <textarea class="widefat"
                      ...
                      name="<?php echo esc_attr(
                      	$instance,
                      ); ?>"><?php echo esc_html($instance); ?></textarea>
            <?php if (!empty($note)): ?>
                <small><?php echo esc_html($note); ?></small>
            <?php endif; ?>
        </p>
                ...
                <?php foreach ($elements as $key => $elem): ?>
                    <li>
                        <input type="checkbox" value="<?php echo esc_attr($key); ?>"
                               ...
                               name="<?php echo esc_attr($instance); ?>[]" <?php checked(
	is_array($instance) && in_array($key, $instance),
); ?> />
	...
	public function morenews_generate_image_upload($field, $label, $value, $note = '', $class = '')
	{
        ...
        <div>
            ...
            <div class="image-preview-wrap">
                ...
                <input type="hidden" class="img" name="<?php echo esc_attr(
                	$this->get_field_name($field),
                ); ?>"
                       id="<?php echo esc_attr($this->get_field_id($field)); ?>"
                       value="<?php echo esc_attr($instance); ?>"/>
                <input type="button" class="select-img button button-primary"
                       ...
                       data-uploader_button_text="<?php esc_attr_e(
                       	'Choose Image',
                       	'morenews',
                       ); ?>"/>
                <?php
	...
	public function morenews_sanitize_data($instance, $new_instance)
	{
		if (is_array($this->text_fields)) {
			...
			foreach ($this->text_fields as $field) {
				$instance = array_merge(
					$instance,
					$this->morenews_update_text($field, $new_instance),
				);
			}
		}
		...
		if (is_array($this->url_fields)) {
			...
			foreach ($this->url_fields as $field) {
				$instance = array_merge(
					$instance,
					$this->morenews_update_url($field, $new_instance),
				);
			}
		}
		...
		if (is_array($this->text_areas)) {
			...
			foreach ($this->text_areas as $field) {
				$instance = array_merge(
					$instance,
					$this->morenews_update_textarea($field, $new_instance),
				);
			}
		}
		if (is_array($this->checkboxes)) {
			...
			foreach ($this->checkboxes as $field) {
				$instance = array_merge(
					$instance,
					$this->morenews_update_checkbox($field, $new_instance),
				);
			}
		}
		if (is_array($this->select_fields)) {
			...
			foreach ($this->select_fields as $field) {
				$instance = array_merge(
					$instance,
					$this->morenews_update_select($field, $new_instance),
				);
			}
		}
		return $instance;
	...
	{
		$instance = [];
		$instance[$name] = !empty($new_instance[$name])
	...
	public function morenews_update_textarea($name, $new_instance)
	{
		$instance = [];
		$instance[$name] = !empty($new_instance[$name])
			? sanitize_textarea_field($new_instance[$name])
			: '';
		return $instance;
	}
		...
		if (!empty($new_instance[$name])) {
			...
			if (is_array($new_instance[$name])) {
				...
				foreach ($new_instance[$name] as $key => $value) {
					$instance[$name][$key] = !empty($new_instance[$name][$key])
						? esc_attr($value)
						: '';
				}
			} else {
