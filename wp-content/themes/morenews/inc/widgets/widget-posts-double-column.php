...
if (!class_exists('MoreNews_Express_Posts_Double_Column')):
	...
	class MoreNews_Express_Posts_Double_Column extends MoreNews_Widget_Base
	{
		...
		function __construct()
		{
			...
			$this->select_fields = ['morenews-select-category-1', 'morenews-select-category-2'];
			...
			parent::__construct(
				'morenews_posts_double_column',
				__('AFTMN Post Double Columns', 'morenews'),
				$widget_ops,
			);
		}
		...
		public function widget($args, $instance)
		{
			...
			for ($morenews_i = 1; $morenews_i <= 2; $morenews_i++) {
				$morenews_section_title = apply_filters(
					'widget_title',
					$instance['morenews-posts-list-title-' . $morenews_i],
					$instance,
					$this->id_base,
				);
				$morenews_category = !empty($instance['morenews-select-category-' . $morenews_i])
					? $instance['morenews-select-category-' . $morenews_i]
					: '0';
			...
			if (isset($morenews_featured_categories)): ?>
                ...
                <div class="af-container-row pad-v clearfix">
                    <?php foreach ($morenews_featured_categories as $morenews_fc): ?>
                        <div class="col-2 pad float-l af-sec-post">
                            ...
                            <?php $morenews_all_posts_vertical = morenews_get_posts(
                            	3,
                            	$morenews_fc[0],
                            ); ?>
                            <div class="full-wid-resp af-widget-body">
                                ...
                                if ($morenews_all_posts_vertical->have_posts()):
                                	$morenews_count = 1;
                                	while ($morenews_all_posts_vertical->have_posts()):
                                		$morenews_all_posts_vertical->the_post();
                                		global $post;
                                		if ($morenews_count == 1): ?>
		...
		public function form($instance)
		{
			...
			if (isset($categories) && !empty($categories)) {
				...
				echo parent::morenews_generate_select_options(
					...
					__('Select Category 1', 'morenews'),
					$categories,
				);
				echo parent::morenews_generate_select_options(
					'morenews-select-category-2',
					__('Select Category 2', 'morenews'),
					$categories,
				);
			}
