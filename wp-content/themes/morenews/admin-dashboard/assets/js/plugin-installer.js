...
jQuery(document).ready(function ($) {
		...
		$.ajax({
			...
			success: function (data) {
				if (data && data.status === 'success') {
					aft.attr('class', 'activate button button-primary').html(
						aft_installer_localize.activate_btn
					);
				} else {
					aft.removeClass('installing');
				}
	...
	n.activate_plugin = function (aft, plugin) {
		...
		$.ajax({
			...
			success: function (data) {
				if (data && data.status === 'success') {
					aft.attr('class', 'installed button disabled').html(
						aft_installer_localize.installed_btn
					);

					if (data.plugin === 'templatespare') {
						aft.attr('href', data.redirectUrl)
	...
	$(document).on('click', '.aft-bulk-active-plugin-installer a.button', function (e) {
		e.preventDefault();
