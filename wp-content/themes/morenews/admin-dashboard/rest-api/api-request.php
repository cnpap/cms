...
{
	register_rest_route('aft-useful-plugins/v1', '/get-useful-plugins', [
		'methods' => 'GET',
...
function morenews_get_all_useful_plugins(\WP_REST_Request $request)
{
	...
	foreach ($plugin_array as $plugin) {
		...
		if (!is_wp_error($api)) {
			...
			if (morenews_check_file_extension($main_plugin_file)) {
			...
			$data['plugins'][] = morenews_render_plugin_lists_template(
				$plugin,
				$api,
				$button_text,
				$button_classes,
...
function morenews_render_plugin_lists_template(
	$plugin,
	$api,
	$button_text,
	$button_classes,
      ...
      <ul class="activation-row">
        <li>
          <?php if ($api->slug == 'templatespare' && $button_text == 'Activated') { ?>
            <a class="button-primary" href="<?php echo site_url() .
            	'/wp-admin/admin.php?page=morenews'; ?>"><?php echo _e(
	'Get Starter Sites',
	'morenews',
); ?></a>

          <?php } elseif ($api->slug == 'templatespare' && $button_text == 'Install Now') { ?>

            <a class="<?php echo $button_classes; ?>"
              data-slug="<?php echo $api->slug; ?>"
              data-name="<?php echo $api->name; ?>"
              href="<?php echo get_admin_url(); ?>update.php?action=install-plugin&amp;plugin=<?php echo $api->slug; ?>&amp;_wpnonce=<?php echo wp_create_nonce(
	'install-plugin_' . $api->slug,
          ...
          <?php } else { ?>
            ...
            <a class="<?php echo $button_classes; ?>"
              data-slug="<?php echo $api->slug; ?>"
              data-name="<?php echo $api->name; ?>"
              href="<?php echo get_admin_url(); ?>update.php?action=activate-plugin&amp;plugin=<?php echo $api->slug; ?>&amp;_wpnonce=<?php echo wp_create_nonce(
	'install-plugin_' . $api->slug,
); ?>">
              <?php echo $button_text; ?>
            </a>
