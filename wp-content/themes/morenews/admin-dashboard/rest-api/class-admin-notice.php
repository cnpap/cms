...
class AdminNotice
{
	...
	function morenews_admin_notice()
	{
		...
		if (
			$current_screen->id != 'tools' &&
			$current_screen->id != 'plugins' &&
			$current_screen->id != 'options-general' &&
			$current_screen->id !== 'dashboard' &&
			$current_screen->id !== 'themes' &&
			$current_screen->id !== 'appearance_page_af-dashbaord-details'
		) {
			return;
		}
	...
	function morenews_dashboard_notice_content()
	{
		...
		$plugins = apply_filters('aft_plugins_for_starter_sites', ['templatespare']);
		$install_plugin = [];
		...
		$notice_header = sprintf(
			...
			sprintf(
				esc_html__(
					'%s is now active. We\'re here to help you turn your ideas into a beautiful, professional website — quickly and confidently.',
					'morenews',
				),
				$this->theme_name,
			),
		...
		$notice_starter_msg = sprintf(
			'<div class="aft-notice-col-2">
				...
				<div class="aft-general-info-link %9$s ">
					<div>
					<a href="%3$s"  data-install=' .
				json_encode($install_plugin) .
				' data-activate=' .
				json_encode($activate_plugins) .
				' data-page=' .
				esc_html($this->page_slug) .
				' class="button button-primary">%4$s</a>
