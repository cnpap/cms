...
if (!class_exists('AF_themes_info')) {
	class AF_themes_info
	{
		...
		function __construct()
		{
			...
			$this->menu_name = isset($this->config['menu_name'])
				? $this->config['menu_name']
				: $this->theme_name;
			$this->page_name = isset($this->config['page_name'])
				? $this->config['page_name']
				: $this->theme_name;
			$this->page_slug = $this->theme_slug;
			add_action('admin_menu', [$this, 'morenews_register_info_page']);
			add_action('admin_enqueue_scripts', [$this, 'morenews_register_backend_scripts']);
			add_action('init', [$this, 'morenews_load_files']);
			add_filter('admin_body_class', [$this, 'morenews_body_classes']);
			add_action('admin_head', [$this, 'morenews_make_upgrade_link_external']);
    ...
    <?php
		...
		public function morenews_register_info_page()
		{
			...
			add_menu_page(
				...
				'morenews', // Menu slug.
				[$this, 'morenews_render_page'], // Action.
				$afthemes_icon,
				30,
			);
			...
			add_submenu_page(
				...
				'morenews', // Menu slug.
				[$this, 'morenews_render_page'], // Callback function.
				// $get_started_order
			);
			...
			add_submenu_page(
				...
				[$this, 'morenews_render_starter_sites'], // Callback function.
				// $starter_sites_order
			);
			...
			add_submenu_page(
				...
				[$this, 'morenews_render_templates_kits'], // Callback function.
				// $starter_sites_order
			);
			...
			add_submenu_page(
				...
				[$this, 'morenews_render_starter_templates'], // Callback function.
				// $starter_sites_order
			);
      ...
      <?php
		...
		public function morenews_render_starter_sites()
		{
			...
			if (!empty($morenews_templatespare_installed)) {
				$morenews_templatespare_info = get_plugin_data(
					WP_PLUGIN_DIR . '/' . $morenews_templatespare_installed,
				);
			...
			if (
				$morenews_templatespare_active == false &&
				$morenews_templatespare_installed != null
			) {
				$activate[] = 'templatespare';
			}
			$plugin_update = 'false';
			if (
				!empty($morenews_templatespare_verison) &&
				$morenews_templatespare_verison < self::templatespare_old_version
			) {
				$plugin_update = 'true';
			}

			if (
				$morenews_templatespare_installed &&
				$morenews_templatespare_active &&
				$plugin_update == 'false'
			) { ?>
      ...
      <?php } else {
				...
				if (
					...
					$message = __(
						'The Templatespare plugin should be updated to the latest version',
						'morenews',
					);
				} else {
					$class = 'false';
					$message = __(
						'Import a Starter Site, Personalize, and Live it in 3 Easy Steps!',
						'morenews',
					);
				}
				?>
        <div id="templatespare-plugin-install-activate" data-class=<?php echo $class; ?> current-theme=<?php echo esc_attr(
 	$this->theme_slug,
 ); ?> install=<?php echo json_encode($install); ?> activate=<?php echo json_encode(
 	$activate,
 ); ?> data-plugin-page='starter-sites' message='<?php echo $message; ?>' ispro=''></div>
      <?php }
		}
		...
		public function morenews_render_starter_templates()
		{
			...
			if (!empty($morenews_blockspare_installed) && $morenews_check_blockspare == 'pro') {
				...
				$morenews_blockspare_pro_info = get_plugin_data(
					WP_PLUGIN_DIR . '/' . $morenews_blockspare_installed,
				);
				...
				if (
					$morenews_blockspare_active == false &&
					$morenews_blockspare_installed != null
				) {
				...
				if (!empty($morenews_blockspare_installed)) {
					$morenews_blockspare_info = get_plugin_data(
						WP_PLUGIN_DIR . '/' . $morenews_blockspare_installed,
					);
			...
			$plugin_update = 'false';
			if (
				!empty($morenews_blockspare_verison) &&
				$morenews_blockspare_verison < $morenews_blockspare_old_version
			) {
				$plugin_update = 'true';
			}

			if (
				$morenews_blockspare_installed &&
				$morenews_blockspare_active &&
				$plugin_update == 'false'
			) { ?>
      ...
      <?php } else {
				...
				if (
					...
					$class = 'false';
					$message = __(
						'One-click Demo Import, Block Editor Ready, No Code Required! Built with Blockspare.',
						'morenews',
					);
				}
				?>
        <div id="templatespare-plugin-install-activate" data-class="<?php echo $class; ?>" current-theme='blockspare' install=<?php echo json_encode(
	$install,
); ?> activate=<?php echo json_encode(
 	$activate,
 ); ?> data-plugin-page="aft-block-patterns" message='<?php echo $message; ?>' isPro='<?php echo esc_attr(
	$morenews_blockspare_status,
); ?>'></div>
<?php }
		}
		...
		public function morenews_render_templates_kits()
		{
			...
			if (!empty($morenews_elespare_installed) && $morenews_check_elespare == 'pro') {
				...
				$morenews_elespare_pro_info = get_plugin_data(
					WP_PLUGIN_DIR . '/' . $morenews_elespare_installed,
				);
				...
				if (!empty($morenews_elespare_installed)) {
					$morenews_elespare_info = get_plugin_data(
						WP_PLUGIN_DIR . '/' . $morenews_elespare_installed,
					);
					...
					if (
						$morenews_elespare_active == false &&
						$morenews_elespare_installed != null
					) {
						$activate[] = 'elespare';
					}
				} else {
			...
			$plugin_update = 'false';
			if (
				!empty($morenews_elespare_verison) &&
				$morenews_elespare_verison < $morenews_elespare_old_version
			) {
				$plugin_update = 'true';
			}

			if (
				$morenews_elespare_installed &&
				$morenews_elespare_active &&
				$plugin_update == 'false' &&
				is_plugin_active($morenews_elementor_installed)
			) {
				...
				$message =
					...
					$morenews_elespare_verison < $morenews_elespare_old_version
						? sprintf(
							__('Elespare plugin version should be more than %s.', 'morenews'),
							$morenews_elespare_old_version,
						)
						: __(
							'One-click Import, Header/Footer Builder, Multilingual Support! Powered by Elespare.',
							'morenews',
						);
				$class =
					!empty($morenews_elespare_verison) &&
					...
					$morenews_elespare_verison < $morenews_elespare_old_version
						...
						: 'false';
				echo '<div id="templatespare-plugin-install-activate" data-class="' .
					esc_attr($class) .
					'" current-theme="elespare" install="' .
					esc_attr(json_encode($install)) .
					'" activate="' .
					esc_attr(json_encode($activate)) .
					'" data-plugin-page="aft-template-kits" message="' .
					esc_attr($message) .
					'" isPro="' .
					esc_attr($morenews_elespare_status) .
					'"></div>';
			}
		}
		...
		function morenews_register_backend_scripts()
		{
			...
			$morenews_file_modified_time = filemtime(
				get_template_directory() . '/admin-dashboard/dist/admin_dashboard.build.js',
			);
			...
			wp_enqueue_style(
				...
				get_template_directory_uri() . '/admin-dashboard/dist/style-admin_dashboard.css',
				'',
				$morenews_version_with_timestamp,
				'all',
			);
			wp_register_style(
				'templatespare',
				get_template_directory_uri() . '/admin-dashboard/dist/blocks.editor.build.css',
				'',
				$morenews_version_with_timestamp,
				'all',
			);
			wp_enqueue_script(
				...
				['react', 'react-dom', 'wp-api-fetch', 'wp-element'], // Dependencies, defined above.
				'1.0.0',
				true,
			);
			...
			$morenews_templatespare_installed = morenews_check_file_extension(
				'templatespare/templatespare.php',
			);
			$morenews_templatespare_active = is_plugin_active('templatespare/templatespare.php');
			...
			wp_localize_script('aftheme-dashboard', 'afDashboardData', [
				...
				'templatespare_install' => $morenews_templatespare_installed
					? []
					: json_encode(['templatespare']),
				'templatespare_active' => $morenews_templatespare_active
					? []
					: json_encode(['templatespare']),
				'admindashboarddata' => $aft_get_starter_plugins,
				'theme_img' =>
					get_template_directory_uri() . '/admin-dashboard/assets/images/theme.png',
				'externalUrl' =>
					'https://raw.githubusercontent.com/afthemes/elespare-demo-data/master/free',
				'starter_sites' =>
					get_template_directory_uri() .
					'/admin-dashboard/assets/images/starter-sites.jpg',
				'block_patterns' =>
					get_template_directory_uri() .
					'/admin-dashboard/assets/images/block-patterns.jpg',
				'template_kits' =>
					get_template_directory_uri() .
					'/admin-dashboard/assets/images/template-kits.jpg',
			]);

			wp_enqueue_script(
				'plugin-installer',
				get_template_directory_uri() . '/admin-dashboard/dist/plugin_installer.build.js',
				['jquery', 'aftheme-dashboard'],
			);
			wp_enqueue_script(
				'templatespare-installer',
				get_template_directory_uri() .
					'/admin-dashboard/dist/templatespare_plugin.build.js',
				['jquery', 'aftheme-dashboard'],
			);
			wp_localize_script('plugin-installer', 'aft_installer_localize', [
		...
		public function morenews_load_files()
		{
			...
			require_once get_template_directory() .
				'/admin-dashboard/rest-api/class-admin-notice.php';
		...
		{
			$plugins = apply_filters('aft_plugins_for_starter_sites', [
				'blockspare',
				'templatespare',
			]);
			...
			if (empty($activate_plugins) && empty($install_plugin)) {
				...
				$morenews_templatespare_subtitle = __(
					'The "Get Started" action will install/activate the Templatespare and Blockspare plugins for Starter Sites and Templates.',
					'morenews',
				);
			}
		...
		public function morenews_check_blockspare_free_pro_activated()
		{
			...
			if (
				!empty($morenews_blockspare_free_installed) &&
				is_plugin_active($morenews_blockspare_free_installed)
			) {
				$flag = 'free';
			} elseif (
				!empty($morenews_blockspare_pro_installed) &&
				!is_plugin_active($morenews_blockspare_pro_installed)
			) {
				$flag = 'pro';
			} elseif (
				!empty($morenews_blockspare_pro_installed) &&
		...
		public function morenews_check_elespare_free_pro_activated()
		{
			...
			if (
				!empty($morenews_elespare_free_installed) &&
				is_plugin_active($morenews_elespare_free_installed)
			) {
				$flag = 'free';
			} elseif (
				!empty($morenews_elespare_pro_installed) &&
				!is_plugin_active($morenews_elespare_pro_installed)
			) {
				$flag = 'pro';
			} elseif (
				!empty($morenews_elespare_pro_installed) &&
