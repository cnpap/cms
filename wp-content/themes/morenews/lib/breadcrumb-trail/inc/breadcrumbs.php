...
class Breadcrumb_Trail
{
	...
	public function trail()
	{
		...
		if (0 < $item_count) {
			...
			$breadcrumb .= sprintf(
				'<meta name="numberOfItems" content="%d" />',
				absint($item_count),
			);
	...
	protected function set_post_taxonomy()
	{
		...
		$this->post_taxonomy = apply_filters(
			'breadcrumb_trail_post_taxonomy',
	...
	protected function add_items()
	{
		...
		$this->items = array_unique(
			apply_filters('breadcrumb_trail_items', $this->items, $this->args),
		);
	}
	...
	protected function add_front_page_items()
	{
		...
		if (
			...
			is_paged() ||
			(is_singular() && 1 < get_query_var('page'))
		) {
			...
			elseif (true === $this->args['show_title']) {
				$this->items[] =
					is_multisite() && true === $this->args['network']
						? get_bloginfo('name')
						: $this->labels['home'];
			}
		}
	...
	protected function add_singular_items()
	{
		...
		if ($post_title = single_post_title('', false)) {
			if (
				...
				is_paged() ||
				(get_option('page_comments') && 1 < absint(get_query_var('cpage')))
			) {
				$this->items[] = sprintf(
					'<a href="%s">%s</a>',
					esc_url(get_permalink($post_id)),
	...
	protected function add_term_archive_items()
	{
		...
		if (false !== $taxonomy->rewrite) {
			...
			if (
				isset($taxonomy->rewrite['with_front']) &&
	...
	protected function add_minute_hour_archive_items()
	{
		...
		if (true === $this->args['show_title']) {
			$this->items[] = sprintf(
				$this->labels['archive_minute_hour'],
				get_the_time(
					esc_html_x('g:i a', 'minute and hour archives time format', 'morenews'),
				),
			);
		}
	}
	...
	protected function add_minute_archive_items()
	{
		...
		if (true === $this->args['show_title']) {
			$this->items[] = sprintf(
				$this->labels['archive_minute'],
				get_the_time(esc_html_x('i', 'minute archives time format', 'morenews')),
			);
		}
	}
	...
	protected function add_hour_archive_items()
	{
		...
		if (true === $this->args['show_title']) {
			$this->items[] = sprintf(
				$this->labels['archive_hour'],
				get_the_time(esc_html_x('g a', 'hour archives time format', 'morenews')),
			);
		}
	}
	...
	protected function add_day_archive_items()
	{
		...
		$year = sprintf(
			$this->labels['archive_year'],
			get_the_time(esc_html_x('Y', 'yearly archives date format', 'morenews')),
		);
		$month = sprintf(
			$this->labels['archive_month'],
			get_the_time(esc_html_x('F', 'monthly archives date format', 'morenews')),
		);
		$day = sprintf(
			$this->labels['archive_day'],
			get_the_time(esc_html_x('j', 'daily archives date format', 'morenews')),
		);
	...
	protected function add_week_archive_items()
	{
		...
		$week = sprintf(
			$this->labels['archive_week'],
			get_the_time(esc_html_x('W', 'weekly archives date format', 'morenews')),
		);
		...
		if (is_paged()) {
			$this->items[] = esc_url(
				get_archives_link(
					add_query_arg(['m' => get_the_time('Y'), 'w' => get_the_time('W')], home_url()),
					$week,
					false,
				),
			);
				...
				if ('%year%' == $tag) {
					$this->items[] = sprintf(
						...
						sprintf(
							$this->labels['archive_year'],
							get_the_time(
								esc_html_x('Y', 'yearly archives date format', 'morenews'),
							),
						),
					);
				}
				...
				elseif ('%monthnum%' == $tag) {
					$this->items[] = sprintf(
						...
						sprintf(
							$this->labels['archive_month'],
							get_the_time(
								esc_html_x('F', 'monthly archives date format', 'morenews'),
							),
						),
					);
				}
