	...
	<div class="akismet-lower">
		...
		<?php if (!empty($notices)) { ?>
			<?php foreach ($notices as $notice) { ?>
				<?php Akismet::view('notice', array_merge($notice, ['parent_view' => $name])); ?>
			<?php } ?>
		<?php } ?>
  	...
  	?>
					<div class="akismet-new-snapshot__chart">
						<iframe id="stats-iframe" allowtransparency="true" scrolling="no" frameborder="0" style="width: 100%; height: 220px; overflow: hidden;" src="<?php echo esc_url(
      	sprintf(
      		'https://tools.akismet.com/1.0/snapshot.php?blog=%s&token=%s&height=200&locale=%s&is_redecorated=1',
      		rawurlencode(get_option('home')),
      		rawurlencode(Akismet::get_access_token()),
      		get_user_locale(),
      	),
      ); ?>" name="<?php echo esc_attr(
	'snapshot-' . filemtime(__FILE__),
); ?>" title="<?php echo esc_attr__('Akismet stats', 'akismet'); ?>"></iframe>
					</div>
					...
					<ul class="akismet-new-snapshot__list">
						<li class="akismet-new-snapshot__item">
							...
							<span class="akismet-new-snapshot__number"><?php echo number_format(
       	$stat_totals['6-months']->spam,
       ); ?></span>
							<span class="akismet-new-snapshot__text"><?php echo esc_html(
       	...
       	_n('Spam blocked', 'Spam blocked', $stat_totals['all']->spam, 'akismet'),
						...
						<li class="akismet-new-snapshot__item">
							...
							<span class="akismet-new-snapshot__number"><?php echo floatval(
       	$stat_totals['all']->accuracy,
       ); ?>%</span>
       ...
       echo esc_html(
       	sprintf(
       		_n('%s missed spam', '%s missed spam', $stat_totals['all']->missed_spam, 'akismet'),
       ...
       echo esc_html(
       	sprintf(
       		_n(
       			'%s false positive',
       			'%s false positives',
       			$stat_totals['all']->false_positives,
       			'akismet',
       		),
     ...
     ); ?>" autocomplete="off" method="POST" id="akismet-settings-form">
							...
							<?php if (!Akismet::predefined_api_key()): ?>
								<div class="akismet-settings__row">
									<h3 class="akismet-settings__row-title">
										<label class="akismet-settings__row-label" for="key"><?php esc_html_e(
          	'API key',
          	'akismet',
          ); ?></label>
									</h3>
									<div class="akismet-settings__row-input">
										<span class="api-key"><input id="key" name="key" type="text" size="15" value="<?php echo esc_attr(
          	get_option('wordpress_api_key'),
          ); ?>" class="<?php echo esc_attr(
	'regular-text code ' . $akismet_user->status,
); ?>"></span>
									</div>
								</div>
							<?php endif; ?>
       ...
       if (isset($_GET['ssl_status'])): ?>
									...
									<div class="akismet-settings__row-text">
										...
										<div class="akismet-settings__row-description">
											<?php if (!wp_http_supports(['ssl'])): ?>
												...
												<?php esc_html_e(
            	'Your Web server cannot make SSL requests; contact your Web host and ask them to add support for SSL requests.',
            	'akismet',
            ); ?>
											<?php else: ?>
												...
												<?php if ($ssl_disabled): ?>
													...
													<?php esc_html_e(
             	'Akismet encountered a problem with a previous SSL request and disabled it temporarily. It will begin using SSL for requests again shortly.',
             	'akismet',
             ); ?>
												<?php else: ?>
							...
							<div class="akismet-settings__row">
								...
								<div class="akismet-settings__row-input">
									<label class="akismet-settings__row-input-label" for="akismet_show_user_comments_approved">
										...
										value="1"
										type="checkbox"
										<?php // If the option isn't set, or if it's enabled ('1'), or if it was enabled a long time ago ('true'), check the checkbox.
          checked(
          	true,
          	in_array(
									...
									<fieldset>
										...
										<div>
											<label class="akismet-settings__row-input-label" for="akismet_strictness_1">
												<input type="radio" name="akismet_strictness" id="akismet_strictness_1" value="1" <?php checked(
            	'1',
            	get_option('akismet_strictness'),
            ); ?> />
												<span class="akismet-settings__row-label-text">
													<?php esc_html_e(
             	...
             	'akismet',
										...
										<div>
											<label class="akismet-settings__row-input-label" for="akismet_strictness_0">
												<input type="radio" name="akismet_strictness" id="akismet_strictness_0" value="0" <?php checked(
            	'0',
            	get_option('akismet_strictness'),
            ); ?> />
												<span class="akismet-settings__row-label-text">
          ...
          $delete_message = _n(
          	'Spam in the %1$s older than %2$d day is deleted automatically.',
          	'Spam in the %1$s older than %2$d days is deleted automatically.',
          	$delete_interval,
          	'akismet',
          );
          ...
          ?>
									...
									<fieldset>
										...
										<div>
											<label class="akismet-settings__row-input-label" for="akismet_comment_form_privacy_notice_display">
												<input type="radio" name="akismet_comment_form_privacy_notice" id="akismet_comment_form_privacy_notice_display" value="display" <?php checked(
            	'display',
            	get_option('akismet_comment_form_privacy_notice'),
            ); ?> />
												<span class="akismet-settings__row-label-text">
										...
										<div>
											<label class="akismet-settings__row-input-label" for="akismet_comment_form_privacy_notice_hide">
												<input type="radio" name="akismet_comment_form_privacy_notice" id="akismet_comment_form_privacy_notice_hide" value="hide" <?php echo in_array(
            	get_option('akismet_comment_form_privacy_notice'),
            	['display', 'hide'],
            	true,
            )
            	? checked('hide', get_option('akismet_comment_form_privacy_notice'), false)
            	: 'checked="checked"'; ?> />
												<span class="akismet-settings__row-label-text">
         	...
         	Akismet_Admin::get_page_url('delete_key'),
         ); ?>"><?php esc_html_e('Disconnect this account', 'akismet'); ?></a>
								</div>
							<?php endif; ?>
							...
							<div id="publishing-action">
								...
								<input type="submit" name="submit" id="submit" class="akismet-button akismet-could-be-primary" value="<?php esc_attr_e(
        	'Save changes',
        	'akismet',
        ); ?>">
							</div>
						...
						<table class="akismet-account">
							<tbody>
								...
								<tr>
									...
									<td>
										<?php if ('cancelled' === $akismet_user->status):
          ...
          elseif ('no-sub' === $akismet_user->status):
						...
						<div class="akismet-card-actions">
							<?php if ($akismet_user->status === 'active'): ?>
								<div class="akismet-card-actions__secondary-action">
									<a href="https://akismet.com/account" class="akismet-settings__external-link" aria-label="Account overview on akismet.com"><?php esc_html_e(
         	'Account overview',
         	'akismet',
         ); ?></a>
								</div>
        	...
        	'text' =>
        		$akismet_user->account_type === 'free-api-key' && $akismet_user->status === 'active'
        			? __('Upgrade', 'akismet')
        			: __('Change', 'akismet'),
        	'redirect' => 'upgrade',
        ]); ?>
