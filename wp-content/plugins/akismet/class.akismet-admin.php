...
class Akismet_Admin
{
	 ...
	 * @var array
	 */
	private static $activation_banner_pages = [
		'edit-comments.php',
	...
	public static function init_hooks()
	{
		...
		add_filter(
			'plugin_action_links_' . plugin_basename(plugin_dir_path(__FILE__) . 'akismet.php'),
	...
	{
		$settings_link =
			'<a href="' . esc_url(self::get_page_url()) . '">' . __('Settings', 'akismet') . '</a>';
	...
	public static function load_resources()
	{
		...
		if (
			...
			wp_register_style(
				'akismet',
				plugin_dir_url(__FILE__) . $akismet_css_path,
				[],
				self::get_asset_file_version($akismet_css_path),
			);
			wp_enqueue_style('akismet');

			wp_register_style(
				'akismet-font-inter',
				plugin_dir_url(__FILE__) . '_inc/fonts/inter.css',
				[],
				self::get_asset_file_version('_inc/fonts/inter.css'),
			);
			wp_enqueue_style('akismet-font-inter');

			$akismet_admin_css_path = is_rtl()
				? '_inc/rtl/akismet-admin-rtl.css'
				: '_inc/akismet-admin.css';
			wp_register_style(
				'akismet-admin',
				plugin_dir_url(__FILE__) . $akismet_admin_css_path,
				[],
				self::get_asset_file_version($akismet_admin_css_path),
			);
			...
			wp_register_script(
				'akismet.js',
				plugin_dir_url(__FILE__) . '_inc/akismet.js',
				['jquery'],
				self::get_asset_file_version('_inc/akismet.js'),
			);
			wp_enqueue_script('akismet.js');

			wp_register_script(
				'akismet-admin.js',
				plugin_dir_url(__FILE__) . '_inc/akismet-admin.js',
				[],
				self::get_asset_file_version('/_inc/akismet-admin.js'),
			);
	...
	public static function admin_help()
	{
			...
			if (!Akismet::get_api_key() || (isset($_GET['view']) && $_GET['view'] == 'start')) {
				...
				$current_screen->add_help_tab([
					...
					'content' =>
						...
						'<p>' .
						esc_html__(
							'On this page, you are able to set up the Akismet plugin.',
							'akismet',
						) .
						'</p>',
				]);
				...
				$current_screen->add_help_tab([
					...
					'content' =>
						...
						'<p>' .
						sprintf(
							__('Sign up for an account on %s to get an API Key.', 'akismet'),
							'<a href="https://akismet.com/plugin-signup/" target="_blank">Akismet.com</a>',
						) .
						'</p>',
				]);
				...
				$current_screen->add_help_tab([
					...
					'content' =>
						...
						'<ol>' .
						'<li>' .
						esc_html__('Copy and paste the API key into the text field.', 'akismet') .
						'</li>' .
						'<li>' .
						esc_html__('Click the Use this Key button.', 'akismet') .
						'</li>' .
						'</ol>',
				]);
				...
				$current_screen->add_help_tab([
					...
					'content' =>
						...
						'<p>' .
						esc_html__(
							'On this page, you are able to update your Akismet settings and view spam stats.',
							'akismet',
						) .
						'</p>',
				]);
				...
				$current_screen->add_help_tab([
					...
					'content' =>
						...
						'</strong> - ' .
						esc_html__(
							'Show the number of approved comments beside each comment author in the comments list page.',
							'akismet',
						) .
						'</p>' .
						'<p><strong>' .
						...
						'</strong> - ' .
						esc_html__(
							'Choose to either discard the worst spam automatically or to always put all spam in spam folder.',
							'akismet',
						) .
						'</p>',
				]);
				...
				if (!Akismet::predefined_api_key()) {
					$current_screen->add_help_tab([
						...
						'content' =>
							...
							'</strong></p>' .
							'<p><strong>' .
							esc_html__('Subscription Type', 'akismet') .
							'</strong> - ' .
							esc_html__('The Akismet subscription plan', 'akismet') .
							'</p>' .
							'<p><strong>' .
							esc_html__('Status', 'akismet') .
							'</strong> - ' .
							esc_html__(
		...
		$current_screen->set_help_sidebar(
			'<p><strong>' .
				...
				'</strong></p>' .
				'<p><a href="https://akismet.com/faq/" target="_blank">' .
				esc_html__('Akismet FAQ', 'akismet') .
				'</a></p>' .
				'<p><a href="https://akismet.com/support/" target="_blank">' .
	...
	public static function enter_api_key()
	{
		...
		foreach (['akismet_strictness', 'akismet_show_user_comments_approved'] as $option) {
			update_option(
				$option,
				isset($_POST[$option]) && (int) $_POST[$option] == 1 ? '1' : '0',
			);
		}
			...
			sprintf(
				...
				esc_url(
					add_query_arg(
						['page' => 'akismet-admin'],
						admin_url(
							isset($submenu['edit-comments.php']) ? 'edit-comments.php' : 'edit.php',
						),
					),
				),
	...
	public static function rightnow_stats()
	{
		if ($count = get_option('akismet_spam_count')) {
			...
			$intro = sprintf(
				__('<a href="%s">Akismet</a> blocks spam from getting to your blog. ', 'akismet'),
				'https://akismet.com/wordpress/',
			);
		}
		...
		echo '<a
			...
			esc_attr(
				remove_query_arg(
					...
					add_query_arg(['akismet_recheck_error' => 1]),
				),
			) .
			'"
	...
	public static function recheck_queue()
	{
		...
		if (
			!(
				...
				(isset($_REQUEST['action']) && 'akismet_recheck_queue' == $_REQUEST['action'])
			)
		) {
			return;
		}
		...
		$result_counts = self::recheck_queue_portion(
	...
	public static function recheck_queue_portion($start = 0, $limit = 100)
	{
		...
		$moderation = $wpdb->get_col(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->comments} WHERE comment_approved = '0' LIMIT %d OFFSET %d",
				$limit,
				$start,
			),
		);
	...
	public static function comment_row_action($a, $comment)
	{
			...
			foreach ($a as $k => $item) {
				...
				if ($k == 'edit' || $k == 'unspam') {
					$b['history'] =
						...
						esc_attr__('View comment history', 'akismet') .
						'"> ' .
		...
		if ($desc) {
			echo '<span class="akismet-status" commentid="' .
				$comment->comment_ID .
				'"><a href="comment.php?action=editcomment&amp;c=' .
				$comment->comment_ID .
				'#akismet-status" title="' .
		...
		if ($show_user_comments) {
			$comment_count = Akismet::get_user_comments_approved(
				$comment->user_id,
			...
			echo sprintf(
				esc_html(_n('%s approved', '%s approved', $comment_count, 'akismet')),
			...
			foreach ($history as $row) {
				...
				if (!empty($row['message'])) {
					...
					switch ($row['event']) {
						...
						case 'check-spam':
							$message = esc_html(
								__('Akismet caught this comment as spam.', 'akismet'),
							);
							break;
						case 'recheck-ham':
							$message = esc_html(
								__('Akismet re-checked and cleared this comment.', 'akismet'),
							);
							break;
						case 'check-ham':
						...
						case 'check-ham-pending':
							$message = esc_html(
								__('Akismet provisionally cleared this comment.', 'akismet'),
							);
							break;
						case 'wp-blacklisted':
						case 'wp-disallowed':
						...
						case 'report-spam':
							if (isset($row['user'])) {
								...
								$message = esc_html(
									sprintf(
										__('%s reported this comment as spam.', 'akismet'),
										$row['user'],
									),
								);
							} elseif (!$message) {
								$message = esc_html(
									__('This comment was reported as spam.', 'akismet'),
								);
							}
							break;
						case 'report-ham':
							if (isset($row['user'])) {
								...
								$message = esc_html(
									sprintf(
										__('%s reported this comment as not spam.', 'akismet'),
										$row['user'],
									),
								);
							} elseif (!$message) {
								$message = esc_html(
									__('This comment was reported as not spam.', 'akismet'),
								);
							}
							break;
						...
						case 'check-error':
							if (isset($row['meta'], $row['meta']['response'])) {
								...
								$message = sprintf(
									esc_html(
										__(
											'Akismet was unable to check this comment (response: %s) but will automatically retry later.',
											'akismet',
										),
									),
									'<code>' . esc_html($row['meta']['response']) . '</code>',
								);
							} else {
								$message = esc_html(
									__(
						...
						case 'recheck-error':
							if (isset($row['meta'], $row['meta']['response'])) {
								...
								$message = sprintf(
									esc_html(
										__(
											'Akismet was unable to recheck this comment (response: %s).',
											'akismet',
										),
									),
									'<code>' . esc_html($row['meta']['response']) . '</code>',
								...
								$message = esc_html(
									__('Akismet was unable to recheck this comment.', 'akismet'),
								);
							}
							break;
						case 'webhook-spam':
						...
						case 'webhook-ham':
							$message = esc_html(
								__(
									'Akismet cleared this comment and updated its status via webhook.',
									'akismet',
								),
							);
							break;
						case 'webhook-spam-noaction':
							$message = esc_html(
								__(
									'Akismet determined this comment was spam during a recheck. It did not update the comment status because it had already been modified by another user or plugin.',
									'akismet',
								),
							);
							break;
						case 'webhook-ham-noaction':
							$message = esc_html(
								__(
									'Akismet cleared this comment during a recheck. It did not update the comment status because it had already been modified by another user or plugin.',
									'akismet',
								),
							);
							break;
						case 'akismet-skipped':
							$message = esc_html(
								__(
									'This comment was not sent to Akismet when it was submitted because it was caught by something else.',
									'akismet',
								),
							);
							break;
						case 'akismet-skipped-disallowed':
							$message = esc_html(
								__(
									'This comment was not sent to Akismet when it was submitted because it was caught by the comment disallowed list.',
									'akismet',
								),
							);
							break;
						default:
							if (preg_match('/^status-changed/', $row['event'])) {
								...
								$message = sprintf(
									esc_html(__('Comment status was changed to %s', 'akismet')),
									'<code>' . esc_html($new_status) . '</code>',
								);
								...
								if (isset($row['user'])) {
									...
									$message = sprintf(
										esc_html(
											__(
												'%1$s changed the comment status to %2$s.',
												'akismet',
											),
										),
										$row['user'],
				...
				if (!empty($message)) {
					...
					if (isset($row['time'])) {
						...
						$time_html =
							'<span style="color: #999;" alt="' .
							esc_attr($time) .
							'" title="' .
							esc_attr($time) .
							'">' .
							sprintf(
								esc_html__('%s ago', 'akismet'),
	...
	public static function get_spam_count($type = false)
	{
		...
		return (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(comment_ID) FROM {$wpdb->comments} WHERE comment_approved = 'spam' AND comment_type = %s",
				$type,
			),
		);
	}
	...
	public static function check_server_connectivity($cache_timeout = 86400)
	{
		...
		if (
			time() - get_option('akismet_connectivity_time') < $cache_timeout &&
			$servers !== false
		) {
	...
	public static function get_akismet_user($api_key)
	{
		...
		$subscription_verification = Akismet::http_post(
			Akismet::build_query($request_args),
			'get-subscription',
		);
	...
	public static function connect_jetpack_user()
	{
		if ($jetpack_user = self::get_jetpack_user()) {
			if (isset($jetpack_user['user_id']) && isset($jetpack_user['api_key'])) {
				$akismet_user = self::verify_wpcom_key(
					$jetpack_user['api_key'],
					$jetpack_user['user_id'],
	...
	public static function display_spam_check_warning()
	{
		...
		if (
			wp_next_scheduled('akismet_schedule_cron_recheck') > time() &&
			self::are_any_comments_waiting_to_be_checked()
		) {
			/*
	...
	public static function display_configuration_page()
	{
		...
		if (get_option('akismet_strictness') === false) {
			add_option(
				'akismet_strictness',
				get_option('akismet_discard_month') === 'false' ? '0' : '1',
			);
		}
		...
		if (empty(self::$notices)) {
			if (
				!empty($stat_totals['all']) &&
				isset($stat_totals['all']->time_saved) &&
				$akismet_user->status == 'active' &&
				...
				if ($stat_totals['all']->time_saved > 1800) {
					...
					if ($total_in_days > 1) {
						...
						$time_saved =
							...
							sprintf(
								_n(
									'Akismet has saved you %s day!',
									'Akismet has saved you %s days!',
									$total_in_days,
									'akismet',
								),
								number_format_i18n($total_in_days),
							);
						...
						$time_saved =
							...
							sprintf(
								_n(
									'Akismet has saved you %d hour!',
									'Akismet has saved you %d hours!',
									$total_in_hours,
									'akismet',
								),
								$total_in_hours,
						...
						$time_saved =
							...
							sprintf(
								_n(
									'Akismet has saved you %d minute!',
									'Akismet has saved you %d minutes!',
									$total_in_minutes,
									'akismet',
								),
								$total_in_minutes,
		...
		if (
			!Akismet::predefined_api_key() &&
			!isset(self::$notices['status']) &&
			in_array($akismet_user->status, ['cancelled', 'suspended', 'missing', 'no-sub'])
		) {
	...
	public static function display_notice()
	{
		...
		if (
			in_array($hook_suffix, ['edit-comments.php']) &&
			...
			in_array($hook_suffix, self::$activation_banner_pages, true) &&
			!Akismet::get_api_key()
		) {
		...
		if (isset($_GET['akismet_recheck_complete'])) {
			...
			if ($recheck_count === 0) {
				...
				$message = sprintf(
					_n(
						'Akismet checked %s comment.',
						'Akismet checked %s comments.',
						$recheck_count,
						'akismet',
					),
				...
				if ($spam_count === 0) {
					...
					$message .= sprintf(
						_n(
							'%s comment was caught as spam.',
							'%s comments were caught as spam.',
							$spam_count,
							'akismet',
						),
						number_format($spam_count),
					);
				}
			}
	...
	public static function modify_plugin_description($all_plugins)
	{
		if (isset($all_plugins['akismet/akismet.php'])) {
			if (Akismet::get_api_key()) {
				$all_plugins['akismet/akismet.php']['Description'] = __(
				...
				$all_plugins['akismet/akismet.php']['Description'] = __(
					'Used by millions, Akismet is quite possibly the best way in the world to <strong>protect your blog from spam</strong>. It keeps your site protected even while you sleep. To get started, just go to <a href="admin.php?page=akismet-key-config">your Akismet Settings page</a> to set up your API key.',
					'akismet',
				);
			}
		}
	...
	public static function erase_personal_data($email_address, $page = 1)
	{
		...
		foreach ((array) $comments as $comment) {
			$comment_as_submitted = get_comment_meta(
				$comment->comment_ID,
				'akismet_as_submitted',
				true,
			);
	...
	protected static function get_inline_css(): string
	{
		...
		$inline_css =
			...
			esc_attr(Akismet_Compatible_Plugins::DEFAULT_VISIBLE_PLUGIN_COUNT + 1) .
			') {
				display: none;
			}
