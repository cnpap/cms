...
class Akismet_CLI extends WP_CLI_Command
{
	...
	public function check($args, $assoc_args)
	{
		foreach ($args as $comment_id) {
			...
			if ('true' === $api_response) {
				...
				/* translators: %d: Comment ID. */
				WP_CLI::warning(
					sprintf(__('Comment #%d could not be checked.', 'akismet'), $comment_id),
				);
			}
		}
	}
	...
	public function recheck_queue()
	{
		...
		WP_CLI::line(
			sprintf(
				...
				number_format($total_counts['processed']),
			),
		);
		...
		WP_CLI::line(
			sprintf(
				_n(
					'%d comment moved to Spam.',
					'%d comments moved to Spam.',
					$total_counts['spam'],
					'akismet',
				),
				number_format($total_counts['spam']),
			),
		);
		...
		if ($total_counts['error']) {
			...
			WP_CLI::line(
				sprintf(
					_n(
						'%d comment could not be checked.',
						'%d comments could not be checked.',
						$total_counts['error'],
						'akismet',
					),
					number_format($total_counts['error']),
				),
			);
		}
	}
	...
	public function stats($args, $assoc_args)
	{
		...
		if (isset($assoc_args['summary'])) {
			$keys = ['spam', 'ham', 'missed_spam', 'false_positives', 'accuracy', 'time_saved'];
