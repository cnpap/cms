...
class Akismet_Compatible_Plugins
{
	 ...
	 * @var array
	 */
	protected const COMPATIBLE_PLUGIN_FIELDS = ['slug', 'name', 'logo', 'help_url', 'path'];
	...
	{
		add_action('activated_plugin', [static::class, 'handle_plugin_change'], true);
		add_action('deactivated_plugin', [static::class, 'handle_plugin_change'], true);
	}
	...
	public static function handle_plugin_change(string $plugin): void
	{
		...
		$plugin_change_should_invalidate_cache = in_array(
			$plugin,
			array_column($cached_plugins, 'path'),
		);
	...
	private static function get_compatible_plugins(): array
	{
		...
		$response = wp_remote_get(self::COMPATIBLE_PLUGIN_ENDPOINT);
	 ...
	 * @return array
	 */
	private static function sanitize_compatible_plugin_response(array $plugins = []): array
	{
	...
	private static function get_cached_plugins()
	{
		...
		return wp_cache_get(static::CACHE_KEY . "_$_blog_id", static::CACHE_GROUP . "_$_blog_id");
	}
