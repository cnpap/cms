...
class Akismet
{
	...
	private static $comment_as_submitted_allowed_keys = [
		'blog' => '',
		'blog_charset' => '',
		'blog_lang' => '',
		'blog_ua' => '',
	...
	private static function init_hooks()
	{
		...
		add_action('wp_insert_comment', ['Akismet', 'schedule_email_fallback'], 10, 2);
		add_action('wp_insert_comment', ['Akismet', 'schedule_approval_fallback'], 10, 2);
		...
		add_action('akismet_email_fallback', ['Akismet', 'email_fallback'], 10, 3);
		add_action('akismet_approval_fallback', ['Akismet', 'approval_fallback'], 10, 3);
		...
		add_filter('notify_moderator', ['Akismet', 'disable_emails_if_unreachable'], 1000, 2);
		add_filter('notify_post_author', ['Akismet', 'disable_emails_if_unreachable'], 1000, 2);
		...
		add_action('xmlrpc_call', ['Akismet', 'pre_check_pingback'], 10, 3);
		...
		add_filter('jetpack_contact_form_akismet_values', [
			'Akismet',
	...
	public static function get_access_token()
	{
		...
		if (is_null($access_token)) {
			$request_args = ['api_key' => self::get_api_key()];
	...
	public static function auto_check_comment($commentdata, $context = 'default')
	{
		...
		if (function_exists('wp_check_comment_disallowed_list')) {
			$comment_author = isset($commentdata['comment_author'])
				...
				: '';
			$comment_author_email = isset($commentdata['comment_author_email'])
				? $commentdata['comment_author_email']
				: '';
			$comment_author_url = isset($commentdata['comment_author_url'])
				? $commentdata['comment_author_url']
				: '';
			$comment_content = isset($commentdata['comment_content'])
				? $commentdata['comment_content']
				: '';
			$comment_author_ip = isset($commentdata['comment_author_IP'])
			...
			if (
				...
				$commentdata['comment_meta']['akismet_skipped_microtime'] =
					$commentdata['akismet_skipped_microtime'];
		...
		if ($akismet_nonce_option == 'true' || $akismet_nonce_option == '') {
			...
			if (
				isset($_POST['akismet_comment_nonce']) &&
				wp_verify_nonce(
		...
		$commentdata['comment_as_submitted'] = array_intersect_key(
			$comment,
			self::$comment_as_submitted_allowed_keys,
		);
		...
		if (isset($response[0]['x-akismet-guid'])) {
			...
			if ('false' === $response[1]) {
				...
				if (isset($response[0]['X-akismet-recheck-after'])) {
					...
					self::log(
						'Delaying moderation email for comment from ' .
							$commentdata['comment_author'] .
							' for ' .
							$delay .
							' seconds',
		...
		if (isset($response[0]['x-akismet-error'])) {
			...
			if ($discard) {
				...
				if ('rest_api' === $context) {
					return new WP_Error(
						'akismet_rest_comment_discarded',
	...
	public static function set_last_comment($comment)
	{
		if (is_null($comment)) {
			...
			self::$last_comment = wp_filter_comment(
				array_merge(['comment_author_IP' => self::get_ip_address()], $comment),
			);
		}
	}
	...
	public static function auto_check_update_meta($id, $comment)
	{
		...
		if (is_object($comment) && !empty(self::$last_comment) && is_array(self::$last_comment)) {
			if (self::matches_last_comment_by_id($id)) {
				...
				if (
					isset(self::$last_comment['akismet_result']) &&
					self::$last_comment['akismet_result'] == 'true'
				) {
					...
					isset(self::$last_comment['akismet_result']) &&
					'skipped' == self::$last_comment['akismet_result']
				) {
	...
	public static function schedule_email_fallback($id, $comment)
	{
		...
		if ($email_delay) {
			...
			wp_schedule_single_event(time() + $email_delay, 'akismet_email_fallback', [$id]);

			self::log(
				'Scheduled email fallback for ' . (time() + $email_delay) . ' for comment #' . $id,
			);
		} else {
	...
	public static function email_fallback($comment_id)
	{
		...
		if (get_comment_meta($comment_id, 'akismet_delayed_moderation_email', true)) {
			self::log(
				'Triggering notification emails for comment #' .
					$comment_id .
					'. They will be sent if comment is not spam.',
			);
	...
	public static function schedule_approval_fallback($id, $comment)
	{
		...
		if ($approval_delay) {
			...
			wp_schedule_single_event(time() + $approval_delay, 'akismet_approval_fallback', [$id]);

			self::log(
				'Scheduled approval fallback for ' .
					(time() + $approval_delay) .
					' for comment #' .
					$id,
			);
		...
		if (wp_get_comment_status($comment_id) == 'unapproved') {
			if (self::last_comment_status_change_came_from_akismet($comment_id)) {
				...
				if (!$comment) {
					...
					self::log(
						'Not approving comment #' .
							$comment_id .
							' because it does not pass check_comment()',
					);
				}
				...
				self::log(
					'No need to fallback approve comment #' .
						$comment_id .
						' because it was not last modified by Akismet.',
				);
	...
	public static function delete_old_comments()
	{
		...
		while (
			$comment_ids = $wpdb->get_col(
				$wpdb->prepare(
					"SELECT comment_id FROM {$wpdb->comments} WHERE DATE_SUB(NOW(), INTERVAL %d DAY) > comment_date_gmt AND comment_approved = 'spam' LIMIT %d",
					$delete_interval,
					$delete_limit,
				),
			)
			...
			$format_string = implode(
				', ',
				array_fill(0, is_countable($comment_ids) ? count($comment_ids) : 0, '%s'),
			);
			...
			do_action(
				...
				is_countable($comment_ids) ? count($comment_ids) : 0,
			);
		}
	...
	public static function delete_orphaned_commentmeta()
	{
		...
		while (
			$commentmeta_results = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT m.meta_id, m.comment_id, m.meta_key FROM {$wpdb->commentmeta} as m LEFT JOIN {$wpdb->comments} as c USING(comment_id) WHERE c.comment_id IS NULL AND m.meta_id > %d ORDER BY m.meta_id LIMIT 1000",
					$last_meta_id,
				),
			)
		) {
	...
	public static function get_user_comments_approved(
		...
		if (is_array($excluded_comment_types) && !empty($excluded_comment_types)) {
			...
			foreach ($excluded_comment_types as $excluded_comment_type) {
				$comment_type_where .= $wpdb->prepare(
					' AND comment_type <> %s ',
	 ...
	 */
	public static function update_comment_history(
		$comment_id,
		$message,
		$event = null,
		$meta = null
	...
	public static function check_db_comment($id, $recheck_reason = 'recheck_queue')
	{
		...
		$c = $wpdb->get_row(
			$wpdb->prepare("SELECT * FROM {$wpdb->comments} WHERE comment_ID = %d", $id),
			ARRAY_A,
		);
	...
	public static function recheck_comment($id, $recheck_reason = 'recheck_queue')
	{
		...
		if (is_wp_error($api_response)) {
			...
			self::update_comment_history($id, '', 'recheck-error', [
	...
	public static function transition_comment_status($new_status, $old_status, $comment)
	{
		...
		if (
			...
			(isset($_POST['status']) &&
				in_array($_POST['status'], ['spam', 'unspam', 'approved'])) ||
			...
			(isset($_POST['comment_status']) &&
				in_array($_POST['comment_status'], ['spam', 'unspam'])) ||
			...
			(isset($_GET['action']) &&
				in_array($_GET['action'], ['spam', 'unspam', 'spamcomment', 'unspamcomment'])) ||
			...
			if (
				$new_status == 'spam' &&
				($old_status == 'approved' || $old_status == 'unapproved' || !$old_status)
			) {
				return self::submit_spam_comment($comment->comment_ID);
			} elseif (
				$old_status == 'spam' &&
				($new_status == 'approved' || $new_status == 'unapproved')
			) {
	...
	public static function submit_spam_comment($comment_id)
	{
		...
		$as_submitted = self::sanitize_comment_as_submitted(
			get_comment_meta($comment_id, 'akismet_as_submitted', true),
		);
	...
	public static function cron_recheck()
	{
		...
		if (get_option('akismet_alert_code') || $status == 'invalid') {
			...
			do_action(
				...
				'key-problem-' . get_option('akismet_alert_code') . '-' . $status,
			);
			return false;
		}
		...
		$comment_errors = $wpdb->get_col(
			"SELECT comment_id FROM {$wpdb->commentmeta} WHERE meta_key = 'akismet_error'	LIMIT 100",
		);
		...
		foreach ((array) $comment_errors as $comment_id) {
			...
			if (!empty($event)) {
				...
				if (
					intval(gmdate('U')) - strtotime($comment->comment_date_gmt) <
		...
		$remaining = $wpdb->get_var(
			"SELECT COUNT(*) FROM {$wpdb->commentmeta} WHERE meta_key = 'akismet_error'",
		);
	...
	private static function comments_match($comment1, $comment2)
	{
		...
		if (!empty($comment1['akismet_guid']) && !empty($comment2['akismet_guid'])) {
			...
			!empty($comment1['akismet_skipped_microtime']) &&
			!empty($comment2['akismet_skipped_microtime'])
		) {
			...
			return strval($comment1['akismet_skipped_microtime']) ==
				strval($comment2['akismet_skipped_microtime']);
		}
	...
	public static function get_fields_for_comment_matching($comment_id)
	{
		return [
			...
			'akismet_skipped_microtime' => get_comment_meta(
				$comment_id,
	...
	public static function http_post($request, $path, $ip = null)
	{
		...
		$alert_code_check_paths = ['verify-key', 'comment-check', 'get-stats'];
	...
	public static function get_akismet_form_fields()
	{
		...
		$fields .=
			'<p style="display: none !important;" class="akismet-fields-container" data-prefix="' .
			esc_attr($prefix) .
			'">';
		$fields .=
			'<label>&#916;<textarea name="' .
			$prefix .
			'hp_textarea" cols="45" rows="8" maxlength="100"></textarea></label>';
		...
		if (!function_exists('amp_is_request') || !amp_is_request()) {
			...
			$fields .=
				...
				'"/>';
			$fields .=
				'<script>document.getElementById( "ak_js_' .
				$field_count .
				'" ).setAttribute( "value", ( new Date() ).getTime() );</script>';
		}
	...
	{
		if (
			'fluentform/form_element_start' === current_filter() &&
			did_action('fluentform_form_element_start')
		) {
	...
	{
		if (
			'fluentform/akismet_fields' === current_filter() &&
			did_filter('fluentform_akismet_fields')
		) {
...
<p><?php echo esc_html($message); ?></p>
  ...
  exit();
	...
	public static function plugin_activation()
	{
		if (version_compare($GLOBALS['wp_version'], AKISMET__MINIMUM_WP_VERSION, '<')) {
			$message =
				...
				sprintf(
					esc_html__('Akismet %1$s requires WordPress %2$s or higher.', 'akismet'),
					AKISMET_VERSION,
			...
			false !== strpos($_SERVER['SCRIPT_NAME'], '/wp-admin/plugins.php')
		) {
	...
	public static function plugin_deactivation()
	{
		...
		$akismet_cron_events = ['akismet_schedule_cron_recheck', 'akismet_scheduled_delete'];
	 ...
	 */
	public static function pre_check_pingback($method, $args = [], $server = null)
	{
		...
		if (!is_null($server) && !empty($args[1])) {
			...
			if ('system.multicall' === $server->message->methodName) {
				$is_multicall = true;
				$multicall_count = is_countable($server->message->params)
					? count($server->message->params)
					: 0;
			}
	...
	public static function display_comment_form_privacy_notice()
	{
		if (
			...
			apply_filters(
				...
				get_option('akismet_comment_form_privacy_notice', 'hide'),
			)
		) {
			return;
		}
	...
	public static function load_form_js()
	{
		if (
			...
			wp_register_script(
				'akismet-frontend',
				plugin_dir_url(__FILE__) . '_inc/akismet-frontend.js',
				[],
