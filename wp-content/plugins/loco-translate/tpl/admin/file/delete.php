...
$this->extend('../layout'); ?> 
    ...
    <form action="" method="post" enctype="application/x-www-form-urlencoded" id="loco-del">
        <div class="panel panel-danger">
            ...
            </p><?php if ($params->has('deps')): ?> 
            <p>
                <strong><?php $params->e('warn'); ?></strong><?php foreach ($deps as $info): ?> 
                <div><?php $info->e('name'); ?></div><?php endforeach; ?> 
            </p><?php endif; ?> 
            <p class="submit">
                <button type="submit" class="button button-danger" disabled><?php esc_html_e(
                	'Delete Permanently',
                	'loco-translate',
                ); ?></button>
            </p>
        </div>
        <?php /* @var Loco_mvc_HiddenFields $hidden */
        $hidden->_e(); ?> 
    </form>
