...
class LocoDomQueryFilter
{
	...
	public function __construct(string $value)
	{
		...
		if (!empty($r[2])) {
			$this->attr['id'] = substr($r[2], 1);
		}
		if (!empty($r[3])) {
			$this->attr['class'] = substr($r[3], 1);
		}
		if (!empty($r[4])) {
			$this->attr[$r[5]] = $r[6];
		}
	}
	public function filter(DOMElement $el): iterable
	{
		if ('' !== $this->tag) {
		...
		if ($this->attr) {
			$list = $this->reduce($list, new ArrayIterator(), $recursive)->getArrayCopy();
		}
		return $list;
	}
	public function reduce(
		DOMNodeList $list,
		ArrayIterator $reduced,
		bool $recursive
	): ArrayIterator {
		foreach ($list as $node) {
			if ($node instanceof DOMElement) {
				...
				if ($recursive && $node->hasChildNodes()) {
					$this->reduce($node->childNodes, $reduced, true);
				}
			}
		}
		return $reduced;
...
{
	public static function parse(string $source): DOMDocument
	{
		...
		$opts = LIBXML_HTML_NODEFDTD;
		$parsed = $dom->loadHTML($source, $opts);
	...
	public function __construct($value)
	{
		...
		if (is_iterable($value)) {
			...
			$nodes = [$value->documentElement];
		} else {
			$type = is_object($value) ? get_class($value) : gettype($value);
			throw new InvalidArgumentException('Cannot construct DOM from ' . $type);
		}
		parent::__construct($nodes);
	}
	public function eq($index): self
	{
		$q = new LocoDomQuery([]);
		if ($el = $this[$index]) {
			$q[] = $el;
	...
	public function find($value): self
	{
		$q = new LocoDomQuery([]);
		$f = new LocoDomQueryFilter($value);
		foreach ($this as $el) {
	...
	public function hasClass(string $class): bool
	{
		foreach ($this as $el) {
			...
			if (is_string($classes) && false !== strpos($classes, $class)) {
				return true;
			}
		}
		return false;
	...
	{
		parse_str($this->serializeForm(), $data);
		return $data;
	}
	public function serializeForm(): string
	{
		$pairs = [];
		foreach (['input', 'select', 'textarea', 'button'] as $type) {
			foreach ($this->find($type) as $field) {
				...
				if ('select' === $type) {
					$value = null;
					$f = new LocoDomQueryFilter('option');
					...
					$value = '';
				} elseif ($field->hasAttribute('value')) {
					$value = $field->getAttribute('value');
				} else {
