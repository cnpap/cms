...
class LocoHeaders extends ArrayIterator implements LocoArrayInterface
{
	...
	public function normalize(string $k): ?string
	{
		$k = strtolower($k);
		return array_key_exists($k, $this->map) ? $this->map[$k] : null;
	}
	public function add($key, $val): self
	{
	...
	public function trimmed(string $prop): string
	{
...
function loco_normalize_charset(string $cs): string
{
	if (preg_match('/^UTF-?(8|16-?(LE|BE)?)$/i', $cs, $r, PREG_UNMATCHED_AS_NULL)) {
		return '8' === $r[1] ? 'UTF-8' : 'UTF-16' . $r[2];
	}
	try {
		return mb_preferred_mime_name($cs);
	} catch (ValueError $e) {
		try {
			if (preg_match('/^csISO(\\w+)/i', $cs, $r) || preg_match('/^(\\w+)8$/', $cs, $r)) {
				return mb_preferred_mime_name($r[1]);
			}
			throw $e;
		} catch (ValueError $e) {
...
class LocoPoHeaders extends LocoHeaders
{
	...
	public function setCharset(string $to): string
	{
	...
	public static function fromMsgstr(string $str): LocoPoHeaders
	{
	...
	public static function fromSource(string $raw): LocoPoHeaders
	{
...
function loco_convert_utf8(string $str, string $enc, bool $strict): string
{
	if ('' === $enc || 'UTF-8' === $enc || 'US-ASCII' === $enc) {
		if (false === preg_match('//u', $str)) {
			if ($strict) {
				$e = new Loco_error_ParseException(
					$enc ? 'Invalid ' . $enc . ' encoding' : 'Unknown character encoding',
				);
				if (
					preg_match(
...
function loco_fix_utf8(string $str): string
{
	$fix = '';
	while (is_string($str) && '' !== $str) {
		if (
			...
			$fix .= mb_convert_encoding($str[0], 'UTF-8', 'Windows-1252');
			$str = substr($str, 1);
		}
	}
...
{
	/*LocoPoHeaders*/ private $head = null;
	/*string*/ private $cs = '';
	abstract public function parse(int $limit = -1): array;
	protected function setHeader(LocoPoHeaders $head): LocoPoHeaders
	{
		$this->head = $head;
		$cs = $head->getCharset();
		if ('' !== $cs) {
	...
	public function getHeader(): ?LocoPoHeaders
	{
		return $this->head;
	}
	protected function setCharset(string $cs): void
	{
		$this->cs = $cs;
	}
	...
	protected function str(string $str): string
	{
		if ('' !== $str) {
	...
	protected function initMsgKey(string $key): array
	{
		$r = explode("\4", $key);
...
function loco_remove_bom(string $s, &$c): string
{
	$bom = substr($s, 0, 2);
	if ("\xFF\xFE" === $bom) {
		$c = 'UTF-16LE';
...
function loco_parse_reference_id(string $refs, &$_id): string
{
...
class LocoPoParser extends LocoGettextParser implements Iterator
{
		...
		while (array_key_exists(++$i, $this->lines)) {
			...
			try {
				...
				if ('#' === $c) {
					...
					preg_match('/^msg(id(?:_plural)?|ctxt|str(?:\\[(\\d+)])?)[ \\t]*/', $line, $r)
				) {
					if (isset($r[2])) {
						$key = 'str';
						$idx = (int) $r[2];
	...
	public function parse(int $limit = -1): array
	{
		$this->rewind();
		if (!$this->valid()) {
		...
		if (-1 === $i) {
			throw new Loco_error_ParseException('Invalid PO file');
		} elseif (0 === $i && '' === $assets[0]['source'] && '' === $assets[0]['target']) {
			throw new Loco_error_ParseException('Invalid PO file');
		}
		return $assets;
	}
...
class LocoMoParser extends LocoGettextParser
{
	...
	public function parse(int $limit = -1): array
	{
		...
		while ($soffset < $targetOffset) {
			...
			if (isset($sources[1])) {
				...
				for ($n = 1; $n < $nforms; $n++) {
					$r[++$i] = [
						'source' => 1 === $n && isset($sources[1]) ? $this->str($sources[1]) : '',
	...
	public function targetOffset(): int
	{
		if (is_null($this->t)) {
	...
	private function words(int $offset, int $length): string
	{
		return $this->bytes($offset * 4, $length * 4);
	}
	private function integerWord(int $offset): int
	{
		return $this->integerAt($offset * 4);
	}
	private function integerAt(int $offset): int
	{
...
abstract class LocoPo
{
	public static function pair(
		...
		if ($nbr) {
		} elseif ($width && $width < mb_strlen($text, 'UTF-8') + strlen($key) + 3) {
		} else {
			return $key . ' "' . $text . '"';
		}
		$lines = [$key . ' "'];
	...
	private static function replace_br(array $r): string
	{
...
{
	public function compare(LocoPoMessage $a, LocoPoMessage $b): int
	{
		$h = $a->getHash();
		if (!isset($this[$h])) {
			return 1;
...
class LocoPoMessage extends ArrayObject
{
	...
	private function getPoFlags(): array
	{
		$flags = [];
	...
	public function exportSerial(string $f = 'target'): array
	{
		$a = [$this[$f]];
		if ($this->offsetExists('plurals')) {
	...
	public function render(int $width, int $ref_width, int $max_forms = 0): string
	{
		$s = [];
		try {
	...
	public function merge(LocoPoMessage $def, bool $translate = false): void
	{
	...
	private function diff(string $key, LocoPoMessage $def, array $prev): array
	{
		$old = $this->__get($key);
		$new = $def->__get($key);
		if ($new !== $old) {
	...
	private function getPlural(int $i): ?self
	{
		if ($this->offsetExists('plurals')) {
			$plurals = $this->offsetGet('plurals');
	...
	private function exportBasic(): array
	{
...
{
	/*array*/ private $po;
	/*LocoPoHeaders*/ private $headers = null;
	/*int*/ private $i;
	/*int*/ private $t;
	/*int*/ private $j;
	/*int*/ private $z = 0;
	/*int*/ private $w = 79;
	public function __construct(iterable $po)
	{
		if (is_array($po)) {
			$this->po = $po;
	...
	public function push(LocoPoMessage $p): void
	{
		$raw = $p->export();
		$plurals = $p->plurals;
		unset($raw['plurals']);
	...
	public function concat(iterable $more): self
	{
	...
	#[ReturnTypeWillChange]
	public function current(): LocoPoMessage
	{
		return $this->item($this->i);
	}
	private function item(int $i): LocoPoMessage
	{
		$po = $this->po;
	...
	public function exportEntry(int $i): LocoPoMessage
	{
	...
	public function setHeaders(LocoPoHeaders $head): self
	{
		$this->headers = $head;
		if (0 === $this->z) {
	...
	public function render(?callable $sorter = null): string
	{
		$width = $this->w;
	...
	public function exportJed(): array
	{
	...
	public function exportRefs(string $grep = ''): array
	{
		$a = [];
		if ('' === $grep) {
			$grep = '/(\\S+):\\d+/';
	...
	public function splitRefs(?array $map = null): array
	{
		$a = [];
		$self = get_class($this);
		$base = [$this->exportHeader()];
	...
	public function equalSource(LocoPoIterator $that): bool
	{
		return $this->getHashes() === $that->getHashes();
	}
	public function equal(LocoPoIterator $that): bool
	{
		if ($this->t !== $that->t) {
			return false;
		}
	...
	public function sort(?callable $func = null): self
	{
		$order = [];
		foreach ($this as $msg) {
			$order[] = $msg;
	...
	public static function compare(LocoPoMessage $a, LocoPoMessage $b): int
	{
		$h = $a->getHash();
		$j = $b->getHash();
		$n = strcasecmp($h, $j);
	...
	public function createSorter(): array
	{
		$index = [];
...
class LocoMoTable
{
	...
	private function reset(int $length): int
	{
	...
	public function compile(array $msgids): void
	{
	...
	public function lookup(string $msgid, array $msgids): int
	{
		$hash_val = self::hashpjw($msgid);
	...
	private function parse(string $bin): void
	{
		$this->bin = $bin;
		$this->size = null;
	...
	public static function hashpjw(string $str): int
	{
		$i = -1;
		$hval = 0;
		$len = strlen($str);
	...
	private static function next_prime(float $seed): int
	{
		$seed = (int) floor($seed);
		$seed |= 1;
	...
	private static function is_prime(int $num): bool
	{
		if (1 === $num) {
			return false;
		}
...
{
	/*string*/ private $bin;
	/*Iterator*/ private $msgs;
	/*LocoPoHeaders*/ private $head;
	/*LocoMoTable*/ private $hash = null;
	/*bool*/ private $use_fuzzy = false;
	/*string*/ private $cs = null;
	...
	public function setCharset(string $cs): void
	{
		$cs = $this->head->setCharset($cs);
		$this->cs = 'UTF-8' === $cs ? null : $cs;
	}
	public function enableHash(): void
	{
		$this->hash = new LocoMoTable();
	}
	public function useFuzzy(): void
	{
		$this->use_fuzzy = true;
	}
	public function setHeader(string $key, string $val): self
	{
	...
	public function compile(): string
	{
		...
		if (
			...
			preg_match('/^nplurals=(\\d)/', $this->head->trimmed('Plural-Forms'), $r)
		) {
			$nplurals = (int) $r[1];
		...
		foreach ($this->msgs as $r) {
			...
			if (array_key_exists($msgid, $unique)) {
				continue;
			}
			$unique[$msgid] = true;
	...
	private function writeInteger(int $num): void
	{
		$this->bin .= pack('V', $num);
	}
}
...
{
	/*int*/ const T_LITERAL = 0;
	/*int*/ const T_UNKNOWN = -1;
	/*string*/ private $src;
	/*int*/ private $pos;
	/*int*/ private $line;
	/*int*/ private $col;
	...
	public function generate(string $src): Generator
	{
		$this->init($src);
		while ($this->valid()) {
			yield $this->current();
	...
	public function init(string $src): self
	{
		$this->src = $src;
		$this->rewind();
		return $this;
	}
	public function define(string $grep, /*mixed*/ $t = 0): self
	{
		if ('^' !== $grep[1]) {
	...
	public function ignore(...$symbols): LocoTokensInterface
	{
	...
	{
		$this->skip = array_diff_key($this->skip, array_fill_keys($symbols, true));
		return $this;
	}
	#[ReturnTypeWillChange]
	...
	public function next(): void
	{
		...
		while ($offset <= $this->max) {
			...
			if (is_null($t)) {
				...
				if (false === $n) {
					$s = $text[0];
					$match = [mb_convert_encoding($s, 'UTF-8', 'Windows-1252')];
				}
				$s = (string) $match[0];
				$t = self::T_UNKNOWN;
			}
...
function loco_resolve_surrogates(string $s)
{
...
function _loco_resolve_surrogates(array $r)
{
	return loco_utf8_chr(
		((((832 | (ord($r[1]) & 0x3f)) << 6) | (ord($r[2]) & 0x3f)) - 0xd800) * 0x400 +
			((((832 | (ord($r[3]) & 0x3f)) << 6) | (ord($r[4]) & 0x3f)) - 0xdc00) +
			0x10000,
	);
}
class LocoEscapeParser
{
	...
	final public function unescape(string $s): string
	{
		if ('' !== $s) {
	...
	final public function unescapeMatch(array $r): string
	{
		$s = $r[0];
		$c = $s[1];
		if (isset($this->map[$c])) {
	...
	protected function stripSlashes(string $s): string
	{
...
{
	/*LocoEscapeParser*/ private static $lex = null;
	/*array*/ protected static $words = [
		'true' => 1,
		'false' => 1,
		'null' => 1,
		'break' => T_BREAK,
	...
	public static function decapse(string $encapsed): string
	{
	...
	public function matchWord(string $s): int
	{
	...
	public function matchComment(string $s): int
	{
		if (substr($s, 0, 3) === '/**') {
...
class LocoExtracted implements Countable
{
	...
	public function getDomainCounts(): array
	{
		return $this->dom;
	}
	public function setDomain(string $default): self
	{
		$this->dflt = $default;
		return $this;
	}
	...
	public function pushEntry(array $entry, string $domain): int
	{
		if ('' === $domain || '*' === $domain) {
			$domain = $this->dflt;
		}
	...
	public function pushPlural(array $entry, int $sindex): void
	{
	...
	public function mergeField(array $old, array $new, string $field, string $glue): string
	{
		...
		if (isset($new[$field])) {
			...
			if ('' !== $prev && $prev !== $text) {
				if (
					'notes' === $field &&
					preg_match('/^' . preg_quote(rtrim($text, '. '), '/') . '[. ]*$/mu', $prev)
				) {
					$text = $prev;
				} else {
					$text = $prev . $glue . $text;
	...
	public function filter(string $domain): array
	{
		if ('' === $domain) {
			$domain = $this->dflt;
		}
...
abstract class LocoExtractor implements LocoExtractorInterface
{
	...
	public function headerize(array $tags, string $domain = ''): self
	{
	...
	final public function extractSource(string $src, string $fileref): LocoExtracted
	{
		$strings = new LocoExtracted();
		$this->extract($strings, $this->tokenize($src), $fileref);
		return $strings;
	}
	public function rule(string $keyword): string
	{
	...
	protected function push(
		...
		if ('' !== $comment) {
			...
			if (
				preg_match('/^references?:( *.+:\\d+)*\\s*/mi', $comment, $r, PREG_OFFSET_CAPTURE)
			) {
	...
	protected function utf8(string $str): string
	{
		if (false === preg_match('//u', $str)) {
			$str = mb_convert_encoding($str, 'UTF-8', 'Windows-1252');
		}
...
{
	/*int*/ private $i = null;
	/*array*/ private $tokens;
	/*array*/ private $skip_tokens;
	/*array*/ private $literal_tokens;
	...
	public function literal(...$symbols): self
	{
...
function loco_unescape_php_string(string $s): string
{
	static $l;
	if (is_null($l)) {
...
function loco_decapse_php_string(string $s): string
{
	if ('' === $s) {
		return '';
	}
...
function loco_parse_php_comment(string $comment): string
{
...
function loco_parse_wp_comment(string $block): array
{
	$header = [];
	if ('/*' === substr($block, 0, 2)) {
		$junk = "\r\t/ *";
...
class LocoPHPExtractor extends LocoExtractor
{
	...
	public function decapse(string $raw): string
	{
		return loco_decapse_php_string($raw);
	}
	public function fsniff(string $str): string
	{
		$format = '';
		$offset = 0;
		while (
		...
		while ($tok = $tokens->advance()) {
			...
			if ($depth) {
				if (')' === $s || ']' === $s) {
					...
					} elseif (T_STRING === $t && array_key_exists($s, $this->defs)) {
			...
			} elseif (T_STRING === $t && '(' === $tokens->advance() && ($rule = $this->rule($s))) {
				$ref = $fileref ? $fileref . ':' . $tok[2] : '';
				$depth = 1;
				$args = [];
				$narg = 0;
...
class LocoWpJsonExtractor implements LocoExtractorInterface
{
	...
	private function getType(string $type): stdClass
	{
...
class LocoWpJsonStrings extends ArrayIterator implements LocoTokensInterface
{
	...
	private function walk(/*mixed*/ $tpl, /*mixed*/ $raw): void
	{
...
function loco_wp_extractor(string $type = 'php', string $ext = ''): LocoExtractorInterface
{
	if ('json' === $type) {
...
function loco_string_percent(int $n, int $t): string
{
	if (!$t || !$n) {
		return '0';
	}
	...
	$n = (100 * $n) / $t;
	if ($n > 99) {
		return rtrim(number_format(min($n, 99.9), ++$dp), '.0');
	}
	if ($n < 0.5) {
		$n = max($n, 0.0001);
		do {
...
function loco_print_progress(int $translated, int $untranslated, int $flagged): void
{
...
class LocoFuzzyMatcher implements Countable
{
	...
	public function redundant(): array
	{
		return array_values($this->po);
	}
	public function setFuzziness($s): void
	{
		if ($this->po) {
	...
	public function add(iterable $a): void
	{
	...
	private function key(iterable $a): string
	{
		$source = isset($a['source']) ? (string) $a['source'] : '';
		$context = isset($a['context']) ? (string) $a['context'] : '';
		return $source . "\4" . $context;
	}
	protected function getRef(iterable $a): ?iterable
	{
		$key = $this->key($a);
		return array_key_exists($key, $this->pot) ? $this->pot[$key] : null;
	}
	public function match(iterable $a): ?iterable
	{
		$old = $this->key($a);
	...
	public function exportPo(): LocoPoIterator
	{
		$p = new LocoPoIterator([['source' => '']]);
		$p->concat($this->pot);
		return $p;
	}
	private static function compareDistance(array $a, array $b): int
	{
		return $a[2] - $b[2];
	}
}
