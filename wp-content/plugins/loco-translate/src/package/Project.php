...
class Loco_package_Project
{
	...
	public function __construct(
		Loco_package_Bundle $bundle,
		Loco_package_TextDomain $domain,
		string $name
	) {
		$this->setName($name);
		$this->bundle = $bundle;
	...
	public function findSourceFiles(): Loco_fs_FileList
	{
		...
		foreach ($crawled as $ext => $files) {
			...
			foreach ($files as $file) {
				...
				if (
					'.min' === substr($name, -4) &&
					file_exists($file->dirname() . '/' . substr($name, 0, -4) . '.' . $ext)
				) {
					continue;
				}
	...
	public function findLocaleFiles(string $ext): Loco_fs_FileList
	{
		...
		foreach ($files[$ext] as $file) {
			...
			elseif ($default && $file->hasSuffixOnly()) {
				...
				elseif (($link = $finder->getFollowed($file)) && $link->underThemeDirectory()) {
					$list->addLocalized($file);
				}
	...
	public function initLocaleFiles(Loco_Locale $locale): Loco_fs_FileList
	{
		...
		foreach ($this->getSystemTargets() as $dir) {
			...
			if (
				('/themes' === substr($path, -7) || '/plugins' === substr($path, -8)) &&
				'' !== $domain
			) {
				$path .= '/' . $domain . '-' . $suffix;
			}
