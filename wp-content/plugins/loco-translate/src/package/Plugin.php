...
class Loco_package_Plugin extends Loco_package_Bundle
{
					...
					foreach ($list as $handle => $data) {
						if (isset($cached[$handle])) {
							Loco_error_AdminNotices::debug(
								sprintf('Plugin conflict on %s', $handle),
							);
							continue;
						}
	...
	public static function get_plugin(string $handle): ?array
	{
		...
		if (!array_key_exists('basedir', $data)) {
			Loco_error_AdminNotices::debug(
				sprintf('"basedir" property required to resolve %s', $handle),
			);
			return null;
		}
	...
	public static function create(string $handle): Loco_package_Plugin
	{
		...
		if (!$data) {
			...
			throw new Loco_error_Exception(
				sprintf(__('Plugin not found: %s', 'loco-translate'), $handle),
			);
		}
		...
		if ($domain = $listener->getDomain($handle)) {
			...
			elseif ($data['TextDomain'] !== $domain) {
				Loco_error_AdminNotices::debug(
					sprintf(
						"Plugin loaded text domain '%s' but WordPress knows it as '%s'",
						$domain,
						$data['TextDomain'],
					),
				);
