...
abstract class Loco_mvc_AdminController extends Loco_mvc_Controller
{
	...
	public function filter_admin_footer_text()
	{
		...
		return '<span id="loco-credit">' .
			sprintf(
				'<span>%s</span> <a href="%s" target="_blank">Loco</a>',
				esc_html(__('Loco Translate is powered by', 'loco-translate')),
				esc_url($url),
			) .
			'</span>';
	}
	...
	public function view($tpl, array $args = [])
	{
		...
		if ($jsConf instanceof Loco_mvc_ViewParams) {
			...
			if (is_object($wp_locale) && property_exists($wp_locale, 'number_format')) {
				$jsConf->offsetSet(
					'wpnum',
					array_map([$this, 'filter_number_format_i18n'], $wp_locale->number_format),
				);
			}
		}
	...
	public function enqueueStyle($name, array $deps = [])
	{
		...
		if (!$base) {
			throw new Loco_error_Exception(
				'Too early to enqueueStyle(' . var_export($name, 1) . ')',
			);
		}
	...
	public function enqueueScript($name, array $deps = [])
	{
		...
		if (!$base) {
			throw new Loco_error_Exception(
				'Too early to enqueueScript(' . json_encode($name) . ')',
			);
		}
