		...
		try {
			...
			if (!$this->ctrl) {
				...
				throw new Loco_error_Exception(
					sprintf(__('Ajax route not found: "%s"', 'loco-translate'), $route),
				);
			}
			...
			if (is_null($json) || '' === $json) {
				throw new Loco_error_Exception(
					__('Ajax controller returned empty JSON', 'loco-translate'),
				);
			}
		} catch (Loco_error_Exception $e) {
			$json = json_encode([
				'error' => $e->jsonSerialize(),
				'notices' => Loco_error_AdminNotices::destroy(),
			]);
		} catch (Exception $e) {
	...
	public function renderDownload()
	{
		try {
			...
			if (is_null($data) || '' === $data) {
				throw new Loco_error_Exception(
					__('Download controller returned empty output', 'loco-translate'),
				);
			}
		} catch (Exception $e) {
			$data = $e;
