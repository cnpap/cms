...
class Loco_admin_file_EditController extends Loco_admin_file_BaseController
{
	...
	public function init()
	{
		...
		$this->set(
			'title',
			sprintf(__('Editing %1$s in %2$s', 'loco-translate'), $file->basename(), $bundle),
		);
	}
	...
	public function render()
	{
		...
		try {
			...
			Loco_error_AdminNotices::debug(
				"Sync is disabled because this file doesn't relate to a known set of translations",
			);
			$project = null;
		}
		...
		if ($locale instanceof Loco_Locale) {
					...
					try {
						...
						if (!$potdata->equalSource($data)) {
							...
							Loco_error_AdminNotices::info(
								sprintf(
									__(
										"Translations don't match template. Run sync to update from %s",
										'loco-translate',
									),
									$potfile->basename(),
								),
								...
								__('Documentation', 'loco-translate'),
							);
						}
						unset($potdata);
					} catch (Exception $e) {
						...
						Loco_error_AdminNotices::warn(
							sprintf(
								__('Translation template is invalid (%s)', 'loco-translate'),
								$potfile->basename(),
							),
						);
						$potfile = null;
			...
			if ($locale) {
				...
				if (2 !== strlen("\xC2\xA3")) {
					Loco_error_AdminNotices::warn(
						'Your mbstring configuration will result in corrupt MO files. Please ensure mbstring.func_overload is disabled',
					);
				}
			}
		}
		...
		if (is_null($locale)) {
			...
			elseif (1 === $settings->pot_protect) {
				$e = new Loco_error_Warning(
					__(
				...
				$e->addLink(
					Loco_mvc_AdminRouter::generate('config') . '#loco--pot-protect',
					__('Settings', 'loco-translate'),
				)
					->addLink(
						apply_filters(
							'loco_external',
