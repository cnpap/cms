...
class Loco_admin_file_DeleteController extends Loco_admin_file_BaseController
{
	...
	private function expandFiles(Loco_fs_File $file)
	{
		try {
			...
			throw new Loco_error_Exception(
				sprintf('Refusing to delete a %s file', strtoupper($ext)),
			);
		}
	...
	public function init()
	{
				...
				try {
					...
					try {
						...
						Loco_data_Session::get()->flash(
							'success',
							sprintf(
								_n('%u file deleted', '%u files deleted', $n, 'loco-translate'),
								$n,
							),
						);
		...
		$this->set(
			'title',
			sprintf(__('Delete %s', 'loco-translate'), $file->basename()) .
				' &lsaquo; ' .
				$bundle->getName(),
		);
	}
			...
			$this->set(
				...
				sprintf(
					_n(
						...
						'%s dependent files will also be deleted',
						$count,
						'loco-translate',
					),
					$count,
