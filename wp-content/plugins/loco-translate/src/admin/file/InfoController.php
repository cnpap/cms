	...
	public function render()
	{
		...
		if ('pot' === $ext || 'po' === $ext || 'mo' === $ext) {
			...
			try {
				...
				($author = $head->trimmed('Last-Translator')) or
				...
				if ($template && !$isTemplate && 'po' === $ext && $template->exists()) {
					try {
						$this->set(
							'potfile',
							new Loco_mvc_FileParams(
								[
									'synced' => Loco_gettext_Data::load($template)->equalSource(
										$data,
									),
								],
								$template,
				...
				if ($debugging) {
					...
					if ($locale) {
						...
						if ('' !== $value) {
							if (
								Loco_Locale::parse($value)->__toString() !== $locale->__toString()
							) {
								// translators: Warning that the language in the file header (1) does not match the file name (2)
								$debug[] = sprintf(
									__(
										'Language header is "%1$s" but file name contains "%2$s"',
										'loco-translate',
									),
									$value,
									$locale,
					...
					if ($project && $head->has('Project-Id-Version')) {
						...
						if (
							false === strpos($inProj, $inHead) &&
							false === strpos($inHead, $inProj)
						) {
							$debug[] = sprintf(
								'Project-Id-Version header is "%s" but project is "%s"',
								$inHead,
								$inProj,
							);
						}
