...
class Loco_admin_file_HeadController extends Loco_admin_file_BaseController
{
		...
		if ($file->exists() && !$file->isDirectory()) {
			...
			while ($this->checkNonce($action)) {
				...
				try {
					...
					Loco_data_Session::get()->flash(
						'success',
						sprintf(
							__('%s file saved', 'loco-translate'),
	...
	public function render()
	{
		...
		if ($conf->mergeJson() && !$potfile->getLocale()->isValid()) {
			Loco_error_AdminNotices::warn(
				'Merging JSON files requires template has a localized file path',
			);
		}
