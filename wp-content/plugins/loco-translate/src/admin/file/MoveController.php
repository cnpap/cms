...
class Loco_admin_file_MoveController extends Loco_admin_file_BaseController
{
	...
	public function init()
	{
			...
			while ($this->checkNonce($action)) {
				...
				foreach ($files->expand() as $source) {
					...
					if (!$api->authorizeMove($source, $target)) {
						Loco_error_AdminNotices::err(
							'Failed to authorize relocation of ' . $source->basename(),
						);
						break 2;
					}
				...
				try {
					if ($count) {
						...
						Loco_data_Session::get()->flash(
							'success',
							sprintf(
								_n('%s file moved', '%s files moved', $total, 'loco-translate'),
								$total,
							),
						);
					}
		...
		$this->set(
			'title',
			sprintf(__('Move %s', 'loco-translate'), $file->basename()) .
				' &lsaquo; ' .
				$bundle->getName(),
		);
	}
