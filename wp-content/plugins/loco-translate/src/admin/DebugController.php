...
class Loco_admin_DebugController extends Loco_mvc_AdminController
{
	...
	private function log(...$args)
	{
		...
		$message = str_replace(
			[LOCO_LANG_DIR, WP_LANG_DIR, WP_CONTENT_DIR, ABSPATH],
			['{loco_lang_dir}', '{wp_lang_dir}', '{wp_content_dir}', '{abspath}'],
			$message,
		);
	...
	private function logDomainState($domain)
	{
		...
		$this->log(
			'$wp_textdomain_registry->has(%s) == %b',
			$domain,
	...
	public function filter_loco_unload_early_textdomain($bool, $domain)
	{
		if ($this->domain === $domain) {
			...
			$this->log(
				'~ filter:loco_unload_early_textdomain: $l10n[%s] => %s; returning %s',
				$domain,
				$type,
				json_encode($bool),
			);
	...
	public function on_loco_unloaded_textdomain($domain)
	{
		if ($domain === $this->domain) {
			$this->log(
				'~ action:loco_unloaded_textdomain: Text domain loaded prematurely, unloaded "%s"',
				$domain,
			);
		}
	}
	...
	public function on_loco_unseen_textdomain($domain)
	{
		...
		if (is_textdomain_loaded($domain)) {
			$this->log(
				'~ action:loco_unseen_textdomain: "%s" was loaded before helper started',
				$domain,
			);
		} else {
			$this->log(
				'~ action:loco_unseen_textdomain: "%s" isn\'t loaded for "%s"',
				$domain,
				$locale,
			);
		}
	...
	public function filter_load_textdomain_mofile($mofile, $domain)
	{
		if ($domain === $this->domain) {
			$this->log(
				'~ filter:load_textdomain_mofile: %s (exists=%b)',
				$mofile,
				file_exists($mofile),
			);
		}
	...
	public function filter_pre_load_script_translations($translations, $file, $handle /*, $domain*/)
	{
		if ('loco-translate-dummy' === $handle && !is_null($translations)) {
			$this->log(
				'~ filter:pre_load_script_translations: Short-circuited with %s value',
	...
	public function filter_load_script_translations($translations, $file, $handle, $domain)
	{
		if ('loco-translate-dummy' === $handle) {
			...
			if (!is_string($translations) || '' === $translations || '{' !== $translations[0]) {
				$this->log(
					'~ filter:load_script_translations: %s',
	...
	public function temp_filter_gettext()
	{
		...
		if ($args[$i] === $this->domain) {
			...
			$this->log(
				'~ filter:gettext: %s => %s',
				json_encode($args, $opts),
				json_encode($translation, $opts),
			);
		}
		return $translation;
	}
	...
	private function parsePluralForms($raw)
	{
		try {
			...
			if (
				!preg_match(
					'#^nplurals=\\d+;\\s*plural=([-+/*%!=<>|&?:()n\\d ]+);?$#',
					$raw,
					$match,
				)
			) {
				throw new InvalidArgumentException(
					'Invalid Plural-Forms header, ' . json_encode($raw),
				);
			}
	...
	private function preloadDomain($domain, $type, $path)
	{
		...
		elseif ('none' === $type || '' === $type) {
			$file = null;
			Loco_error_AdminNotices::debug(
				'Path argument ignored. Not required for this loading option.',
			);
		} else {
			...
			if (isset($GLOBALS['l10n'][$domain])) {
				...
				if (class_exists('WP_Translation_Controller', false)) {
					...
					if ($locale && $locale !== $this->locale) {
						Loco_error_AdminNotices::warn(
							sprintf(
								'Translations already loaded for "%s". A loader is recommended to select "%s"',
								$locale,
								$this->locale,
							),
						);
		...
		if ('plugin' === $type) {
			if ($file) {
				...
				if (!$file->exists() || !$file->isDirectory()) {
					throw new InvalidArgumentException(
						'Loader argument must be a directory relative to WP_PLUGIN_DIR',
					);
				}
			}
			...
			if ($file && (!$file->isAbsolute() || !$file->isDirectory())) {
				throw new InvalidArgumentException(
					'Path argument must reference the theme directory',
				);
			}
			...
			if (!in_array(end($bits), $expected)) {
				throw new InvalidArgumentException(
					'Path argument must end in ' . $this->locale . '.mo',
				);
			}
	 ...
	 */
	private function preloadScript(
		$path,
		string $domain,
		?Loco_package_Bundle $bundle = null
	): Loco_gettext_Data {
		...
		if ('.min.js' === substr($path, -7)) {
			...
			if (!array_key_exists($jsfile->extension(), $valid)) {
				Loco_error_AdminNotices::debug(
					"Script path didn't end with ." . implode('|', array_keys($valid)),
				);
			}
		}
		$hash = md5($path);
		...
		$this->log(
			'Calling load_script_textdomain( %s )',
			trim(json_encode([$handle, $domain], JSON_UNESCAPED_SLASHES), '[]'),
		);
		...
		throw new Loco_error_Exception(
			'load_script_textdomain returned ' . var_export($json, true),
		);
	}
	...
	private function renderResult(Loco_mvc_ViewParams $form)
	{
		...
		if ($actual !== $this->locale) {
			...
			Loco_error_AdminNotices::warn(
				sprintf('Locale %s is overriding %s', $actual, $this->locale),
			);
		}
		...
		if ('' !== $msgid_plural) {
			...
			$message->offsetSet('plurals', [
				new LocoPoMessage(['source' => $msgid_plural, 'target' => '']),
			]);
		}
		...
		$this->log(
			'Calling %s( %s )',
			$callee,
			trim(json_encode($params, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE), '[]'),
		);
		...
		if ('' !== $msgid_plural) {
			...
			$pluralIndex = $this->selectPluralForm(
				$quantity,
				$pluralIndex,
				$this->parsePluralForms($header),
			);
		}
		...
		if (is_string($path) && '' !== $path) {
			try {
				...
				Loco_error_AdminNotices::warn(
					'Script translation failed. Falling back to PHP translation',
				);
			}
		}
		...
		if ($bundle) {
			foreach ($bundle as $project) {
				if (
					...
					$project->getDomain()->getName() === $domain
				) {
		...
		if ('default' !== $domain) {
			...
			foreach ([new Loco_package_Plugin('', ''), new Loco_package_Theme('', '')] as $tmp) {
				foreach ($tmp->getSystemTargets() as $root) {
					$pofiles->add(
						new Loco_fs_LocaleFile(sprintf('%s/%s-%s.po', $root, $domain, $locale)),
					);
				}
			}
		}
		...
		foreach ($pofiles as $pofile) {
			...
			if (isset($exts['po']) && $exts['po'] && !isset($exts['mo'])) {
				Loco_error_AdminNotices::debug(
					'Found in PO, but not MO. Is it fuzzy? Does it need recompiling?',
				);
			}
		}
		...
		$this->log(
			'> %u matches in %u locations; %u files searched',
			$matched,
			count($grouped),
			$searched,
		);
		if ($matches || $translated) {
			...
			$result['calleeDoc'] =
				'https://developer.wordpress.org/reference/functions/' . $callee . '/';
		...
		throw new Loco_error_Warning(
			'No `' . $locale . '` translations found for this string in ' . $name,
		);
	}
	...
	private function surpriseMe()
	{
		...
		$bundles = array_merge(Loco_package_Plugin::getAll(), Loco_package_Theme::getAll(), [
		...
		while ($files) {
			...
			$strings = (new LocoExtracted())
				->extractSource($extractor, $file->getContents())
				->export();
			if ($strings) {
		...
		if (
			is_array($plurals) &&
			array_key_exists(0, $plurals) &&
			$plurals[0] instanceof LocoPoMessage
		) {
			$form['msgid_plural'] = $plurals[0]['source'];
		}
		Loco_error_AdminNotices::info(
			sprintf('Randomly selected "%s". Click Submit to check a string.', $project->getName()),
		);
	}
	...
	public function render()
	{
		...
		try {
			...
			elseif (array_key_exists('randomize', $_GET)) {
				$this->surpriseMe();
			}
