...
class Loco_admin_RootController extends Loco_admin_list_BaseController
{
	...
	public function render()
	{
		...
		$this->set(
			'title',
			sprintf(__('Loco Translate %s', 'loco-translate'), loco_plugin_version()),
		);
		...
		$this->set(
			...
			new Loco_mvc_ViewParams([
				...
				'link' =>
					'<a href="' .
					esc_url(Loco_mvc_AdminRouter::generate('lang-view', ['locale' => $tag])) .
					'">' .
					esc_html($name) .
					'</a>',
				...
				$this->set(
					...
					new Loco_mvc_ViewParams([
						...
						'link' =>
							'<a href="' .
							esc_url(
								Loco_mvc_AdminRouter::generate('lang-view', ['locale' => $tag]),
							) .
							'">' .
							esc_html($name) .
							'</a>',
