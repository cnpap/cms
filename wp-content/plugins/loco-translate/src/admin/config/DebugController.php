...
class Loco_admin_config_DebugController extends Loco_admin_config_BaseController
{
	...
	private function file_params(Loco_fs_File $file)
	{
		...
		return new Loco_mvc_ViewParams([
			'path' => $this->rel_path($file->getPath()),
	...
	public function render()
	{
		...
		if (
			...
			($mods = preg_grep('/^mod_/', apache_get_modules()))
		) {
		...
		$encoding = new Loco_mvc_ViewParams([
			...
			'charset' =>
				$cs . ' ' . (preg_match('/^utf-?8$/i', $cs) ? "\xE2\x9C\x93" : '(not recommended)'),
		...
		$locations = [
			'WP_LANG_DIR' => $this->file_params(
				new Loco_fs_Directory(loco_constant('WP_LANG_DIR')),
			),
			'LOCO_LANG_DIR' => $this->file_params(
				new Loco_fs_Directory(loco_constant('LOCO_LANG_DIR')),
			),
		];
		...
		$locations['PHP sys_temp_dir'] = $this->file_params(
			new Loco_fs_Directory(sys_get_temp_dir()),
		);
		$locations['PHP upload_tmp_dir'] = $this->file_params(
			new Loco_fs_Directory(ini_get('upload_tmp_dir')),
		);
		$locations['PHP error_log'] = $this->file_params(
			new Loco_fs_Directory(ini_get('error_log')),
		);
		...
		if (version_compare(PHP_VERSION, '7.4', '<')) {
			...
			if (get_magic_quotes_gpc()) {
				Loco_error_AdminNotices::info(
					'You have "magic_quotes_gpc" enabled. We recommend you disable this in PHP',
				);
			}
			if (get_magic_quotes_runtime()) {
				Loco_error_AdminNotices::info(
					'You have "magic_quotes_runtime" enabled. We recommend you disable this in PHP',
				);
			}
			if (version_compare(PHP_VERSION, '5.6.20', '<')) {
				Loco_error_AdminNotices::info(
					'Your PHP version is very old. We recommend you upgrade',
				);
			}
			// phpcs:enable
		}
		...
		return $this->view(
			'admin/config/debug',
			compact('breadcrumb', 'versions', 'encoding', 'memory', 'fs', 'locations', 'debug'),
		);
	}
}
