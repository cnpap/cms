...
class Loco_admin_bundle_SetupController extends Loco_admin_bundle_BaseController
{
	...
	public function render()
	{
		...
		elseif ($post->has('xml-setup') && $this->checkNonce('xml-' . $action)) {
			$bundle->clear();
		...
		elseif ($post->has('json-setup') && $this->checkNonce('json-' . $action)) {
			$bundle->clear();
		...
		elseif ($post->has('reset-setup') && $this->checkNonce('reset-' . $action)) {
			$this->resetBundle();
			$bundle = $this->getBundle();
		}
		...
		foreach ($bundle as $project) {
			...
			if (!$potfile) {
				$notices[] = sprintf(
					'No translation template for the "%s" text domain',
					$project->getSlug(),
				);
			}
		}
	...
	private function getDebug(Loco_package_Bundle $bundle)
	{
		...
		if ('meta' === $bundle->isConfigured()) {
			...
			if ($domain) {
				...
				$notes[] = sprintf(
					__('WordPress says the primary text domain is "%s"', 'loco-translate'),
					$domain,
				);
			...
			if ($path) {
				...
				$notes[] = sprintf(
					__('The domain path is declared by the author as "%s"', 'loco-translate'),
					$path,
				);
			} else {
