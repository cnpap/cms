...
class Loco_admin_bundle_ViewController extends Loco_admin_bundle_BaseController
{
	...
	private function createProjectParams(Loco_package_Project $project)
	{
		...
		foreach ($p['po'] as $pair) {
			...
			if ($pair['installed']) {
				$installed[$lc] = true;
			} elseif (!array_key_exists($lc, $installed)) {
				$installed[$lc] = false;
			}
		}
	 ...
	 */
	private function createProjectPairs(
		Loco_package_Project $project,
		Loco_fs_LocaleFileList $po,
		Loco_fs_LocaleFileList $mo
	): array {
	...
	private function createFileParams(
		Loco_package_Project $project,
		Loco_fs_File $file,
		?Loco_Locale $locale = null
	): Loco_mvc_ViewParams {
		...
		return new Loco_mvc_ViewParams([
			...
			'lattr' => $locale
				? 'class="' . $locale->getIcon() . '" lang="' . $locale->lang . '"'
				: '',
			// file info
			'meta' => $meta,
			'name' => $file->basename(),
	...
	public function render()
	{
		...
		if (
			'Hello Dolly' === $bundle->getName() &&
			'hello.php' === basename($bundle->getHandle())
		) {
		...
		if ('file' === $configured || 'internal' === $configured) {
			// presumed complete
		} elseif ($bundle->isTheme() || ($bundle->isPlugin() && !$bundle->isSingleFile())) {
			...
			foreach ($prefixes as $domain => $_bool) {
				$dummy = new Loco_package_Project(
					$bundle,
