...
class Loco_admin_bundle_ConfController extends Loco_admin_bundle_BaseController
{
	...
	public function init()
	{
		...
		try {
			...
			elseif ($post->has('unconf') && $this->checkNonce($nonce->action)) {
				$this->resetBundle();
			}
		} catch (Exception $e) {
		...
		if ($bundle->isTheme() && ($parent = $bundle->getParent())) {
			$this->set(
				...
				new Loco_mvc_ViewParams([
					'name' => $parent->getName(),
					'href' => Loco_mvc_AdminRouter::generate(
						'theme-conf',
						['bundle' => $parent->getSlug()] + $_GET,
					),
				]),
			);
		}
				...
				if ($parent && $parent->hasProject($project)) {
					...
					if ($project === $default) {
						Loco_error_AdminNotices::warn(
							__(
								'Child theme declares the same Text Domain as the parent theme',
								'loco-translate',
							),
						);
					}
