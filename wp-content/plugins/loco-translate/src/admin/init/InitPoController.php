...
class Loco_admin_init_InitPoController extends Loco_admin_bundle_BaseController
{
	...
	public function render()
	{
		...
		try {
			...
			$subhead = sprintf(
				__('Initializing new translations in "%s"', 'loco-translate'),
				$slug ?: $domain,
			);
		...
		if (
			$localeTag &&
			!array_key_exists($localeTag, $installed) &&
			!array_key_exists($localeTag, $locales)
		) {
		...
		if (!$copying) {
			...
			$prompts['skip'] = new Loco_mvc_ViewParams([
				'link' => Loco_mvc_AdminRouter::generate(
					$this->get('_route'),
					$args + ['extract' => '1'],
				),
		...
		elseif (array_key_exists('copy', $prompts) && !$extracting) {
		...
		elseif (2 > Loco_data_Settings::get()->pot_expected) {
				...
				// else offer assignment of a new file
					...
					$this->set(
						...
						new Loco_mvc_ViewParams([
							'link' => Loco_mvc_AdminRouter::generate(
								$this->get('type') . '-conf',
								array_intersect_key($_GET, ['bundle' => '']),
							),
		...
		if (!$copying) {
			if (array_key_exists('copy', $prompts)) {
				...
				$prompt['text'] = sprintf(
					__('Copy %s instead', 'loco-translate'),
					$prompt->__get('name'),
				);
			} else {
			...
			} elseif (array_key_exists('copy', $prompts)) {
				$prompt['title'] = __(
