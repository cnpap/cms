...
class Loco_Locale implements JsonSerializable
{
	...
	public function __construct(string $lang = '', string $region = '', string $variant = '')
	{
		if (1 == func_num_args() && isset($lang[3])) {
			throw new BadMethodCallException(
				'Did you mean Loco_Locale::parse(' . var_export($lang, 1) . ') ?',
			);
		}
	...
	private function getPluralData(): array
	{
		if (is_null($this->plurals)) {
			...
			[$this->pluraleq, $this->plurals] = $db[''][$id];
		}
	...
	{
		[$equation, $forms] = $this->getPluralData();
	...
	{
		if (
			!preg_match('#^nplurals=(\\d);\\s*plural=([-+/*%!=<>|&?:()n\\d ]+);?$#', $str, $match)
		) {
		...
		if (
			$nplurals === count($this->plurals) &&
			self::hashPlural($previous) === self::hashPlural($pluraleq)
		) {
			return;
		}
	...
	public function getFormality(): string
	{
		...
		if ('' === $variant) {
			...
			elseif ($d->offsetExists($tag . '_informal')) {
				if (!$d->offsetExists($tag . '_formal')) {
					$value = 'formal';
				}
			}
		} elseif ('formal' === $variant || 'informal' === $variant) {
			$value = $variant;
		}
