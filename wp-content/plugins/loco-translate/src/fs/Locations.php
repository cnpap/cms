...
class Loco_fs_Locations extends ArrayObject
{
	...
	{
		if (!self::$roots) {
			self::$roots = new Loco_fs_Locations([loco_constant('ABSPATH')]);
		}
		return self::$roots;
	}
	...
	{
		if (!self::$langs) {
			self::$langs = new Loco_fs_Locations([loco_constant('WP_LANG_DIR')]);
		}
		return self::$langs;
	}
	...
	{
		if (!self::$theme) {
			$roots = isset($GLOBALS['wp_theme_directories'])
