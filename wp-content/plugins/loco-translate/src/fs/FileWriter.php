...
class Loco_fs_FileWriter
{
	...
	public function connect(WP_Filesystem_Base $fs, $disconnected = true)
	{
		if ($disconnected && !$fs->connect()) {
			...
			throw new Loco_error_WriteException(
				__('Failed to connect to remote server', 'loco-translate'),
			);
		}
		$this->fs = $fs;
		return $this;
	...
	private function mapPath($path)
	{
		if (!$this->isDirect()) {
			...
			if (false === $virt) {
				throw new Loco_error_WriteException(
					'Failed to find WP_CONTENT_DIR via remote connection',
				);
			}
	...
	public function chmod($mode, $recursive = false)
	{
		...
		if (!$this->fs->chmod($this->getPath(), $mode, $recursive)) {
			...
			throw new Loco_error_WriteException(
				sprintf(__('Failed to chmod %s', 'loco-translate'), $this->file->basename()),
			);
		}
		return $this;
	}
	...
	public function copy(Loco_fs_File $copy)
	{
		...
		if ($copy->exists()) {
			Loco_error_AdminNotices::debug(
				sprintf('Cannot copy %s to %s (target already exists)', $source, $target),
			);
			throw new Loco_error_WriteException(
				__('Refusing to copy over an existing file', 'loco-translate'),
			);
		}
		...
		if (!$this->fs->copy($source, $target, true)) {
			Loco_error_AdminNotices::debug(
				sprintf(
					'Failed to copy %s to %s via "%s" method',
					$source,
					$target,
					$this->fs->method,
				),
			...
			throw new Loco_error_WriteException(
				sprintf(
					__('Failed to copy %1$s to %2$s', 'loco-translate'),
					basename($source),
					basename($target),
				),
			);
	...
	public function delete($recursive = false)
	{
		...
		if (!$this->fs->delete($this->getPath(), $recursive)) {
			...
			throw new Loco_error_WriteException(
				sprintf(__('Failed to delete %s', 'loco-translate'), $this->file->basename()),
			);
		}
	...
	public function putContents($data)
	{
		...
		if ($file->isDirectory()) {
			...
			throw new Loco_error_WriteException(
				sprintf(__('"%s" is a directory, not a file', 'loco-translate'), $file->basename()),
			);
		}
		...
		if (!$fs->put_contents($path, $data, $mode)) {
			...
			if ($file->exists() && !$file->writable()) {
				Loco_error_AdminNotices::debug(
					sprintf(
						'File not writable via "%s" method, check permissions on %s',
						$fs->method,
						$path,
					),
				);
				throw new Loco_error_WriteException(
					__('Permission denied to update file', 'loco-translate'),
				);
			}
			...
			if (!$dir->writable()) {
				Loco_error_AdminNotices::debug(
					sprintf(
						'Directory not writable via "%s" method; check permissions for %s',
						$fs->method,
						$dir,
					),
				);
				throw new Loco_error_WriteException(
					__("Parent directory isn't writable", 'loco-translate'),
				);
			}
			// else reason for failure is not established
			Loco_error_AdminNotices::debug(
				sprintf('Unknown write failure via "%s" method; check %s', $fs->method, $path),
			);
			throw new Loco_error_WriteException(
				__('Failed to save file', 'loco-translate') . ': ' . $file->basename(),
			);
		}
			...
			if ('/' === $parent->getPath() || $parent->readable()) {
				...
				foreach ($stack as $path) {
					if (!$fs->mkdir($path, $mode)) {
						Loco_error_AdminNotices::debug(
							sprintf(
								'mkdir(%s,%03o) failed via "%s" method;',
								var_export($path, 1),
								$mode,
								$fs->method,
							),
						...
						throw new Loco_error_WriteException(
							__('Failed to create directory', 'loco-translate'),
						);
					}
				}
				return true;
	...
	public function authorize()
	{
		if ($this->disabled()) {
			throw new Loco_error_WriteException(
				__('File modification is disallowed by your WordPress config', 'loco-translate'),
			);
		}
		...
		if (1 < $opts->fs_protect && $this->file->getUpdateType()) {
			throw new Loco_error_WriteException(
				__(
					'Modification of installed files is disallowed by the plugin settings',
					'loco-translate',
				),
			);
		}
