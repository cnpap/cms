...
class Loco_fs_FileFinder implements Iterator, Countable, Loco_fs_FileListInterface
{
		...
		while (is_resource($this->dir)) {
			while ($f = readdir($this->dir)) {
				...
				if (is_array($this->exts)) {
					...
					elseif (!array_key_exists($ext, $this->exts)) {
						continue;
					}
				}
	...
	public function add(Loco_fs_File $file): bool
	{
		if (is_array($this->exts)) {
			...
			elseif ($this->symlinks && $this->linked->has($file)) {
				Loco_error_Debug::trace(
					'Symbolic link resolves to a file excluded by filter (%s)',
					$file->basename(),
				);
			} else {
				$glob = implode(',', array_keys($this->exts));
				Loco_error_Debug::trace(
					'Should have filtered out %s when grouping by *.{%s}',
					$file->basename(),
					$glob,
				);
			}
	...
	public function rewind()
	{
		if ($this->cached) {
			...
			foreach ($this->roots as $root) {
				if (
					$root instanceof Loco_fs_Directory &&
					$root->readable() &&
					!$this->isExcluded($root->getPath())
				) {
