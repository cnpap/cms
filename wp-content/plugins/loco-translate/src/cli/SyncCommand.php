		...
		foreach ($projects as $project) {
			...
			if ($potfile && $potfile->exists()) {
				...
				try {
					...
					WP_CLI::error(
						$e->getMessage() . ' in ' . $potfile->getRelativePath($content_dir),
						false,
					);
					$potfile = null;
				}
			...
			foreach ($pofiles as $pofile) {
				...
				try {
					...
					WP_CLI::error(
						$e->getMessage() . ' in ' . $pofile->getRelativePath($content_dir),
						false,
					);
					continue;
				}
				...
				if (!$ref) {
					WP_CLI::warning(
						sprintf(
							'Skipping %s; no valid translation template',
				...
				if ($nfuzzy || $nadded || $ndropped) {
					Loco_cli_Utils::debug(
						'> unchanged:%u added:%u fuzzy:%u dropped:%u',
						$nvalid,
						$nadded,
						$nfuzzy,
						$ndropped,
