		...
		foreach ($projects as $project) {
			...
			if ('core' !== $type) {
				...
				if ($project->getSlug() !== $domain) {
					WP_CLI::warning(
						sprintf(
							'Skipping %s, only single text domain %s supported',
							$project->getId(),
							$type,
						),
					);
			...
			Loco_cli_Utils::debug(
				'Querying WordPress translations API for %s => %s..',
				$type,
				json_encode($args),
			);
			...
			if (!$packages) {
				Loco_cli_Utils::debug(
					'No installable language packs available for %s. Checking if a GlotPress project exists..',
					$project,
				);
				// Ping GlotPress project page. This is the only way we can know if an incomplete project exists
				$response = wp_remote_head(
					sprintf(
						'https://translate.wordpress.org/projects/wp-%s/%s/',
						$type,
						$args['slug'],
					),
				);
				...
				if (404 === $status) {
					WP_CLI::warning(
						sprintf(
							'Skipping %s: 404 from translate.wordpress.org. Probably no GlotPress project.',
							$project,
						),
					);
					continue;
					...
					WP_CLI::warning(
						sprintf(
							'Status %u from translate.wordpress.org. Skipping %s.',
							$status,
							$project,
						),
					);
				...
				Loco_cli_Utils::debug(
					'> Ok, looks like GlotPress project exists; probably no locales above the threshold for a package build',
				);
			}
			...
			foreach ($locales as $locale) {
				...
				if (200 !== $status) {
					WP_CLI::warning(
						sprintf(
							'Status %u from translate.wordpress.org; skipping "%s". Probably no translation team',
							$status,
							$tag,
						),
					);
				...
				if ($pofile->exists()) {
					$info = new Loco_mvc_FileParams([], $pofile);
					Loco_cli_Utils::debug(
						'PO already exists at %s (%s), merging..',
						$info->relpath,
						$info->size,
					);
				...
				// Copy PO directly to disk as per remote source
					...
					$podata->inheritHeader(
						Loco_gettext_Data::dummy()->localize($locale)->getHeaders(),
					);
				}
