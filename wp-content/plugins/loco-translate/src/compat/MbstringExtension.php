...
abstract class Loco_compat_MbstringExtension
{
	public static function mb_detect_encoding(
		$str,
		?array $encoding_list = null,
		bool $strict = false
	): string {
		return !$str || preg_match('//u', $str) ? 'UTF-8' : 'ISO-8859-1';
	}
	...
	public static function mb_strlen($str, ?string $encoding = null): int
	{
		...
		if (!$warned && preg_match('/[\\x80-\\xFF]/', $str)) {
			trigger_error(
				'Character counts will be wrong without mbstring extension',
				E_USER_WARNING,
			);
			$warned = true;
		}
	...
	public static function mb_convert_encoding(
		...
		if ($to_encoding !== $from_encoding && '' !== $str) {
			...
			if (
				'' === $from_encoding ||
				'ISO-8859-1' === $from_encoding ||
				'cp1252' === $from_encoding
			) {
				if (
					'' === $to_encoding ||
					'UTF-8' === $to_encoding ||
					'US-ASCII' === $to_encoding
				) {
...
if (!function_exists('mb_convert_encoding')) {
	...
	{
		return Loco_compat_MbstringExtension::mb_convert_encoding(
			$str,
			$to_encoding,
			(string) $from_encoding,
		);
