...
class Loco_gettext_Matcher extends LocoFuzzyMatcher
{
	...
	private function loadJson(Loco_fs_File $file): int
	{
		...
		if ('' === $ref || !is_string($ref)) {
			$name = $file->basename();
			$ref = preg_match('/-([0-9a-f]{32})\\.json$/', $name, $r)
				? $this->findScript($r[1])
				: '';
			if ('' === $ref) {
		...
		foreach ($jed['locale_data'] as /*$domain =>*/ $keys) {
			foreach ($keys as $msgid => $arr) {
				...
				if ($old) {
					...
					if ('' === $refs) {
						$old['refs'] = $ref;
					} elseif (0 === preg_match('/\\b' . preg_quote($ref, '/') . '\\b/', $refs)) {
						$old['refs'] .= ' ' . $ref;
					}
					$new = $old;
				} else {
				...
				if (
					$this->translate &&
					(!array_key_exists('target', $new) || '' === $new['target'])
				) {
					$new['target'] = $arr[0];
				}
				...
				if (1 < count($arr)) {
					...
					while (array_key_exists(++$index, $arr)) {
						...
						if (
							$this->translate &&
							(!array_key_exists('target', $raw) || '' === $raw['target'])
						) {
	...
	private function getArrayRef(array $a)
	{
		...
		throw new Exception(
			(is_object($r) ? get_class($r) : gettype($r)) .
				' returned from ' .
				get_class($this) .
				'::getRef',
		);
