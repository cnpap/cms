	...
	public function __construct(Loco_package_Bundle $bundle)
	{
		...
		if (!loco_check_extension('tokenizer')) {
			throw new Loco_error_Exception(
				'String extraction not available without required extension',
			);
		}
		$this->bundle = $bundle;
			...
			try {
				...
				if ('php' === $type || 'twig' === $type) {
					...
					if (0 !== $max) {
						...
						if ($size > $max) {
							($list = $this->skipped) or
								($list = $this->skipped = new Loco_fs_FileList());
							$list->add($file);
							continue;
						}
					}
