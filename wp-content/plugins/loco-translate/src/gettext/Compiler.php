...
class Loco_gettext_Compiler
{
	...
	public function writeAll(
		Loco_gettext_Data $po,
		?Loco_package_Project $project = null
	): Loco_fs_FileList {
		$this->writePo($po);
		$this->writeMo($po);
		if ($project) {
		...
		if ($file->exists() && $this->fs) {
			...
			if ($backup && (loco_doing_ajax() || 'cli' === PHP_SAPI) && $backup->exists()) {
				Loco_error_AdminNotices::debug(
					sprintf('Wrote backup: %s -> %s', $file->basename(), $backup->basename()),
				);
			}
		}
	...
	public function writeMo(Loco_gettext_Data $po): int
	{
		try {
			...
			Loco_error_AdminNotices::warn(
				__('PO file saved, but MO file compilation failed', 'loco-translate'),
			);
			$bytes = 0;
		}
	...
	public function writeJson(
		...
		if (is_string($path) && '' !== $path) {
			...
			if (array_key_exists('js', $refs) && $refs['js'] instanceof Loco_gettext_Data) {
				...
				try {
					...
					Loco_error_AdminNotices::warn(
						sprintf(
							__('JSON compilation failed for %s', 'loco-translate'),
							$jsonfile->basename(),
						),
					);
				}
		...
		// continue as per default, generating multiple per-script JSON
			...
			foreach ($po->exportRefs($refsGrep) as $ref => $fragment) {
				...
				if (is_null($use)) {
					Loco_error_AdminNotices::debug(
						sprintf('Skipping JSON for %s; script not found in bundle', $ref),
					);
				}
				// add .js strings to buffer for this json and merge if already present
				elseif (array_key_exists($use, $buffer)) {
			...
			if ($buffer) {
				...
				foreach ($buffer as $ref => $fragment) {
					...
					if ('' === $json) {
						Loco_error_AdminNotices::debug(
							sprintf('Skipping JSON for %s; no translations', $ref),
						);
						continue;
					}
					try {
						...
						Loco_error_AdminNotices::warn(
							sprintf(__('JSON compilation failed for %s', 'loco-translate'), $ref),
						);
					}
				}
				$buffer = null;
			...
			foreach ($this->files->getJsons($domain) as $path) {
				...
				if (!$jsons->has($jsonfile)) {
					try {
						...
						Loco_error_AdminNotices::debug(
							'Unable to remove redundant JSON: ' . $e->getMessage(),
						);
					}
				}
			}
	 ...
	 */
	private static function cloneJson(
		Loco_fs_File $pofile,
		string $ref,
		string $domain
	): Loco_fs_File {
		...
		if (
			$domain &&
			'default' !== $domain &&
			preg_match('/^[a-z]{2,3}(?:_[a-z\\d_]+)?$/i', $name)
		) {
			$name = $domain . '-' . $name;
		}
	...
	public function getSummary(): Loco_mvc_ViewParams
	{
		...
		if ($mobytes && $numjson) {
			Loco_error_AdminNotices::success(
				__('PO file saved and MO/JSON files compiled', 'loco-translate'),
			);
		} elseif ($mobytes) {
			Loco_error_AdminNotices::success(
				__('PO file saved and MO file compiled', 'loco-translate'),
			);
		} else {
			// translators: Success notice where %s is a file extension, e.g. "PO"
			Loco_error_AdminNotices::success(
				sprintf(__('%s file saved', 'loco-translate'), strtoupper($pofile->extension())),
			);
		}
