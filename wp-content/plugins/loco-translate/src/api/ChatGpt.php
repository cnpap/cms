...
abstract class Loco_api_ChatGpt extends Loco_api_Client
{
	...
	public static function process(array $items, Loco_Locale $locale, array $config): array
	{
		...
		Loco_data_CompiledData:
		flush();

		// Build specific prompt for this batch
		$prompt =
			'Translate the `source` properties of the following JSON objects, using the `context` and `notes` properties to identify the meaning';
		...
		$result = wp_remote_request(
			...
			self::init_request_arguments($config, [
				'model' => $model,
				'temperature' => 0,
				...
				'messages' => [
					[
						'role' => 'system',
						'content' =>
							'You are a helpful assistant that translates from ' .
							$sourceLang .
							' to ' .
							$targetLang,
					],
						...
						'schema' => [
							...
							'properties' => [
								'result' => [
									...
									'items' => [
										...
										'properties' => [
											'id' => [
												'type' => 'number',
												'description' =>
													'Corresponding id from the input object',
											],
											'text' => [
												'type' => 'string',
												'description' =>
													'Translation text of the corresponding input object',
											],
										],
										'required' => ['id', 'text'],
									...
									'description' =>
										'Translations of the corresponding input array',
								],
							],
							'required' => ['result'],
							'additionalProperties' => false,
			...
			foreach ($result as $r) {
				...
				if ($ourId !== $gptId) {
					Loco_error_Debug::trace(
						'Bad id field at [%u] expected %s, got %s',
						$i,
						$ourId,
						$gptId,
					);
	...
	private static function decode_response($result): array
	{
		...
		if (200 !== $status) {
			...
			throw new Loco_error_Exception(
				sprintf('OpenAI API returned status %u: %s', $status, $message),
			);
		}
		// all responses have form {choices:[...]}
