...
abstract class Loco_api_DeepL extends Loco_api_Client
{
	...
	public static function process(array $items, Loco_Locale $locale, array $config): array
	{
		...
		$result = wp_remote_request(
			self::baseUrl($api_key) . '/v2/translate',
		...
		if (200 !== $status) {
			...
			throw new Loco_error_Exception(
				sprintf('DeepL returned status %u: %s', $status, $message),
			);
		}
	...
	{
		$data = parent::decodeResponse(
			wp_remote_request(self::baseUrl($key) . '/v2/usage', [
				'redirection' => 0,
				'user-agent' => parent::getUserAgent(),
				'reject_unsafe_urls' => false,
				'headers' => [
