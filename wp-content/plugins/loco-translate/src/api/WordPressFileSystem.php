...
class Loco_api_WordPressFileSystem
{
	...
	public function authorizeCreate(Loco_fs_File $file)
	{
		...
		if ($file->exists()) {
			...
			throw new Loco_error_WriteException(
				sprintf(
					__('%s already exists in this folder', 'loco-translate'),
					$file->basename(),
				),
			);
		}
	...
	{
		$this->preAuthorize($file);
		return ($file->exists() ? $file->writable() : $file->creatable()) ||
			$this->authorize($file);
	}
	...
	private function authorize(Loco_fs_File $file)
	{
		...
		if (!apply_filters('loco_allow_remote', true)) {
			throw new Loco_error_WriteException(
				'Remote connection required, but network access is disabled',
			);
		}
		...
		if ('' === $this->form) {
			Loco_error_AdminNotices::debug(
				'Unknown error capturing output from request_filesystem_credentials',
			);
		}
		return false;
	}
	...
	public function isAutoUpdateDenied()
	{
		...
		if (
			function_exists('wp_is_file_mod_allowed') &&
		...
		if (
			apply_filters('automatic_updater_disabled', loco_constant('AUTOMATIC_UPDATER_DISABLED'))
		) {
			return true;
		}
