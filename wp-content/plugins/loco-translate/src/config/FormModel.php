	...
	private function collectPaths(LocoConfigElement $parent, array $branch)
	{
		...
		foreach ($parent as $child) {
			...
			// @codeCoverageIgnoreStart
				...
				throw new Exception(
					'Unexpected structure: ' . $name . ' not in ' . json_encode($branch),
				);
			}
		...
		foreach ($confs as $conf) {
			...
			if (empty($conf['domain'])) {
				throw new InvalidArgumentException(
					__('Text Domain cannot be empty', 'loco-translate'),
				);
			}
			...
			// project children
			if (
				$nodes = array_intersect_key($conf, [
					'source' => '',
					'target' => '',
					'template' => '',
				])
				...
				if ('file' === $name || 'directory' === $name || 'path' === $name) {
					...
					foreach (
						...
						$child = $parent->appendChild(
							$dom->createElement($ext ? 'file' : 'directory'),
						);
