...
abstract class Loco_test_WordPressTestCase extends WP_UnitTestCase
{
	...
	public function set_up()
	{
		...
		add_filter(
			'loco_missing_plugin',
			[__CLASS__, 'filter_allows_fake_plugins_to_exist'],
			10,
			2,
		);
		...
		add_filter(
			...
			[__CLASS__, 'filter_site_transient_update_core'],
			10,
			1,
		);
		$this->cookies_set = [];
	...
	public function enable_test_plugins()
	{
		...
		add_filter('loco_constant_WPMU_PLUGIN_DIR', [
			__CLASS__,
			'filter_enforce_test_plugins_only',
		]);
	}
