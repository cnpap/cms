...
class Loco_hooks_LoadHelper extends Loco_hooks_Hookable
{
		...
		if ($l10n && is_array($l10n)) {
			...
			foreach (array_keys($l10n) as $domain) {
				if (
					...
					is_string($domain) &&
					'default' !== $domain &&
					apply_filters('loco_unload_early_textdomain', true, $domain)
				) {
			...
			if ($unloaded && loco_debugging()) {
				$n = count($unloaded);
				Loco_error_Debug::trace(
					'Unloaded %u premature text domain%s (%s)',
					$n,
					1 === $n ? '' : 's',
					implode(',', $unloaded),
				);
	...
	public function filter_load_translation_file($file, $domain, $locale)
	{
		...
		elseif (array_key_exists($path, $this->custom)) {
	...
	private function resolveType(string $path): void
	{
		...
		} elseif (Loco_fs_Locations::getThemes()->check($path)) {
	...
	public function filter_load_script_translation_file($path = '', $handle = '')
	{
		...
		elseif (false === $path && array_key_exists($handle, $this->json)) {
	...
	private static function mergeJson(string $json, string $custom): string
	{
		...
		elseif (self::jedValid($overrideJed)) {
			$json = $custom;
		} elseif (!self::jedValid($fallbackJed)) {
			$json = '';
		}
		return $json;
	}
	...
	{
		return is_array($jed) &&
			array_key_exists('locale_data', $jed) &&
			is_array($jed['locale_data']) &&
			$jed['locale_data'];
	}
	 ...
	 */
	public function debug_ngettext(
		$translation = '',
		$single = '',
		$plural = '',
		$number = 0,
	 ...
	 */
	public function debug_gettext_with_context(
		$translation = '',
		$text = '',
		$context = '',
		$domain = ''
	 ...
	 */
	public function debug_ngettext_with_context(
		$translation = '',
		$single = '',
		$plural = '',
		$number = 0,
