	...
	public function __construct(array $data)
	{
		...
		switch ($code) {
			...
			case UPLOAD_ERR_INI_SIZE:
				throw new Loco_error_UploadException(
			...
			case UPLOAD_ERR_NO_FILE:
				throw new Loco_error_UploadException(
					'No file was uploaded, or data is empty',
					$code,
				);
			case UPLOAD_ERR_NO_TMP_DIR:
				throw new Loco_error_UploadException(
					'Missing temporary folder for uploaded file',
					$code,
				);
		...
		if (!Loco_fs_File::is_readable($path)) {
			...
			if ($safe !== $path) {
				Loco_error_AdminNotices::debug(
					sprintf('Moving uploaded file: %s -> %s', $path, $safe),
				);
