	...
	public function render()
	{
		...
		if ($potfile && !$potfile->exists()) {
			...
			if (1 === $conf) {
				...
				Loco_error_AdminNotices::warn(
					sprintf(
						__(
							'Falling back to source extraction because %s is missing',
							'loco-translate',
						),
						$potfile->basename(),
					),
		...
		// else extract POT from source code
			...
			if ($list = $extr->getSkipped()) {
				$n = count($list);
				$maximum = Loco_mvc_FileParams::renderBytes(
					wp_convert_hr_to_bytes(Loco_data_Settings::get()->max_php_size),
				);
				...
				$text = _n(
					...
					'%1$s files over %2$s have been skipped. (Largest is %3$s). Check all strings are present before saving.',
					$n,
					'loco-translate',
				);
