...
class Loco_ajax_ApisController extends Loco_mvc_AjaxController
{
	...
	public function render()
	{
		...
		if (0 === $post->count()) {
			...
			$modal->set(
				...
				new Loco_mvc_ViewParams([
					...
					'href' => apply_filters(
						'loco_external',
						'https://localise.biz/wordpress/plugin/manual/providers',
					),
				]),
			);
			$modal->set(
				...
				new Loco_mvc_ViewParams([
					...
					'href' => apply_filters(
						'loco_external',
						'https://localise.biz/wordpress/translation?l=' . $locale,
					),
				]),
			);
		...
		elseif ('deepl' === $hook && class_exists('Loco_api_DeepL')) {
			$targets = Loco_api_DeepL::process($sources, $locale, $config);
		} elseif ('openai' === $hook && class_exists('Loco_api_ChatGpt')) {
			$targets = Loco_api_ChatGpt::process($sources, $locale, $config);
		} else {
			throw new Loco_error_Exception(
				'API not hooked. Use `add_filter(' . var_export($action, 1) . ',...)`',
			);
		}

		if (count($targets) !== count($sources)) {
			Loco_error_AdminNotices::warn(
				'Number of translations does not match number of source strings',
			);
		}
