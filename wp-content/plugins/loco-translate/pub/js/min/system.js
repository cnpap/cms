...
!(function (d, c) {
	...
	var f = c('#loco-utf8-check')[0].textContent,
		h = d.conf;
	(1 === f.length && 10003 === f.charCodeAt(0)) ||
		d.notices.warn('This page has a problem rendering UTF-8').stick();
	window.ajaxurl && c('#loco-ajax-url').text(window.ajaxurl);
	c('#loco-vers-jquery').text(
		[
			(c.fn && c.fn.jquery) || 'unknown',
			'ui/' + ((c.ui && c.ui.version) || 'none'),
			'migrate/' + (c.migrateVersion || 'none')
		].join('; ')
	);
	d.ajax.post(
		'ping',
		...
		function (a, b, e) {
			a && a.ping
				? c('#loco-ajax-check').text(a.ping)
				: k(e, b, a && a.error && a.error.message);
		},
		k
	);
	f = h.apis;
