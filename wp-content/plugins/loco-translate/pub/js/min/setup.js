...
!(function (r, t, b) {
			...
			success: function (h) {
				if (c) {
					...
					const p = h && h.exact,
						v = h && h.status;
					p
						? ((d['json-content'].value = p),
							b('#loco-remote-empty').hide(),
							b('#loco-remote-found').show())
						: 404 === v
							? l("Sorry, we don't know a bundle by this name")
							: (q.notices.error(h.error || 'Unknown server error'), e());
				}
			},
			error: function () {
				c && (g(), e());
	...
	var q = r.loco,
		n = q.conf || {},
		m,
		d = t.getElementById('loco-remote');
	b(d)
	...
	b.ajax({
		...
		dataType: 'jsonp',
		success: function (a) {
			for (var f = -1, k, e, g = a.length, c = b(d.vendor).html(''); ++f < g; )
				((k = a[f][0]),
					(e = a[f][1]),
					c.append(b('<option></option>').attr('value', k).text(e)));
		},
		cache: !0
	});
