...
!(function (h, g, d) {
	...
	function E(a, b) {
		let c = b.getElementsByTagName('tr'),
			y = c.length;
			...
			: (k(''),
				...
				(f = r.ajax.post(
					...
					function (c, y, q) {
						q === f &&
							(c && c.html
								? ((b = c.html), (A[a] = b), k(b).find('tbody').each(E), p())
								: x((c && c.error) || 'Unknown error'));
					},
					function (c) {
	...
	let f,
		...
		n = 0,
		v = t.paths.length - 2,
		e = g.getElementById('loco-ui');
	h = g.getElementById('loco-fs');
	g = e.getElementsByTagName('form').item(0);
	let D = e.getElementsByTagName('button'),
		w = d(e).find('div.diff-meta'),
		B = D.item(0),
		C = D.item(1);
	h && g && r.fs.init(h).setForm(g);
	v &&
		(d(B).on('click', J).parent().removeClass('jshide'),
		d(C).on('click', I).parent().removeClass('jshide'));
	u(0);
