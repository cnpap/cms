...
!(function (p, u, c) {
	const g = p.loco,
		C = g.po.ref.init(g, g.conf),
		h = u.getElementById('loco-po');
	!(function (d, e) {
		function b() {
		...
		function q(a) {
			x !== a &&
				(c('#loco-content')[a ? 'removeClass' : 'addClass']('loco-invalid'), (x = a));
		}
		let m,
			w,
			l = [],
		...
		t.each(function (a, f) {
			f = c(f);
			f.find('span.po-none').length
				? b()
				: ((w = a),
					null == m && (m = a),
					(a = f.find('.po-text').text()),
		...
		g.watchtext(c(d.parentNode).find('form.loco-filter')[0].q, function (a) {
			if (a) {
				...
				if (z) {
					...
					(q(!1),
						k(0).append(
							c('<li></li>').text(g.l10n._('Nothing matches the text filter'))
						));
				r = !0;
				n();
	...
	var n = (function () {
		let d,
			e = h.clientHeight - 2;
		return function () {
			for (var b = h, k = b.offsetTop || 0; (b = b.offsetParent) && b !== u.body; )
				k += b.offsetTop || 0;
