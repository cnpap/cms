...
switch ($action) {
   ...
   ?>
    ...
    ?>
     ...
     ?>
					<a class="button button-large" href="<?php echo esc_url($change_link); ?>"><?php _e(
	'Update',
); ?></a>
  ...
  break;
	...
	case 'rp':
		...
		login_header(
			__('Reset Password'),
			wp_get_admin_notice(__('Enter your new password below or generate one.'), [
				'type' => 'info',
				'additional_classes' => ['message', 'reset-pass'],
			]),
			$errors,
  ...
  break;
	...
	default:
		...
		$requested_redirect_to =
			isset($_REQUEST['redirect_to']) && is_string($_REQUEST['redirect_to'])
				? $_REQUEST['redirect_to']
				: '';
